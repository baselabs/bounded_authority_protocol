// Package conformance_test is the v2-profile acceptance runner: it executes
// the vendored 268-case corpus-v2 snapshot against the public V2 SDK surface
// only (the same acceptance pattern as the v1 runner), SHA-binds the vendored
...
// index's public_key_fingerprints. Primitive surfaces (json, jcs, base64url,
// jwk, uri, bounds) are version-independent and dispatch to the shared
// surface exactly as the v1 runner does.
...
// certifiedIndexSHA256 is the SHA-256 of the certified v2 corpus index.json
// this SDK was conformed against (base64url). A vendored corpus that hashes
// to anything else fails closed here — silent drift is impossible.
const certifiedIndexSHA256 = "beYom39HsOCnjqRhDnhEoPHVJH2OrOAuyc-YQTCPE9A"

const corpusDir = "corpus-v2"

const totalCases = 268
...
// profile is the successor-major facade under test; the file carries the major.
...
func loadCorpus(t *testing.T) (*corpusIndex, []loadedFile) {
	...
	if idx.TotalCases != totalCases {
		t.Fatalf("index total_cases = %d, want %d", idx.TotalCases, totalCases)
	}
	...
	for _, f := range idx.Files {
		...
		if f.Path == "revision.json" && f.Cases == 0 {
			...
			if format != "bounded-authority-protocol-v2-conformance-corpus-revision" {
...
func TestConformance(t *testing.T) {
	...
	// Successor-major key census (the v1 runner's census adapted to the v2
	// corpus shape): the DECLARED census is an index input from the curated
	// generator, and one curated key emits no case — the case-byte discovery
	// is a strict subset by design. The two-way equality leg is therefore
	// curated == index (both directions); the case-byte walk is the
	// fail-closed subset leg; and every key a VALID verification-surface
	// case declares must be imported at the SDK verify boundary.
	declared := idx.PublicKeyFingerprints
	if !sortedUniqueStrings(declared) {
		t.Fatalf("census: index public_key_fingerprints not sorted/unique")
	}
	curatedRaw, err := os.ReadFile(curatedCensusPath)
	if err != nil {
	...
	if !sortedUniqueStrings(curated) {
	...
	for _, lf := range files {
		for _, c := range lf.cf.Cases {
			collectCaseKeys(t, c.Input, discovery)
		}
	}
	...
	for _, lf := range files {
		for _, c := range lf.cf.Cases {
			if c.Class == "valid" {
				collectCaseKeys(t, c.Input, verifyExpected)
			}
		}
	}
	...
	t.Logf("agreed=%d disagreed=0 census=%d declared=%d", agreed, len(census), len(declared))
}

// curatedCensusPath is the monorepo's curated generator directory — the
// declared census input the index was built from (facts the generator ships
// that are not derivable from case bytes).
const curatedCensusPath = "../../../conformance/generators/curated-inputs-v2.json"

// The census key-label heuristic: a member named like a public key carrying a
// 43-char base64url (32-byte) value; fingerprint/thumbprint/digest/hash
// members are value facts, never import-boundary keys.
var censusKeyLabel = regexp.MustCompile(`(?i)public.*key|key.*public|verification.*key|holder.*key|issuer.*key`)
var censusKeyDeny = regexp.MustCompile(`(?i)fingerprint|thumbprint|digest|hash`)
var censusRawKey = regexp.MustCompile(`^[A-Za-z0-9_-]{43}$`)

func sortedUniqueStrings(list []string) bool {
...
// collectCaseKeys walks one case input tree and records the thumbprint of
// every public-key-labeled raw key member.
func collectCaseKeys(t *testing.T, m map[string]json.RawMessage, target map[string]bool) {
	t.Helper()
	for key, raw := range m {
		if censusKeyDeny.MatchString(key) {
			continue
		}
		if censusKeyLabel.MatchString(key) {
			var s string
			if err := json.Unmarshal(raw, &s); err == nil && censusRawKey.MatchString(s) {
				keyBytes, err := v.Base64urlDecode(s)
				if err == nil && len(keyBytes) == 32 {
					if tp, err := v.PublicKeyThumbprintRaw(keyBytes, nil); err == nil {
...
// runCase dispatches one corpus case to its surface driver. Primitive
// surfaces dispatch to the shared surface; every wire-versioned surface
// dispatches to the V2 façade. Any disagreement is fatal.
func runCase(t *testing.T, c corpusCase) {
	...
	case "proof_signing_input":
		...
		si, err := profile.ProofSigningInput(p, nil)
		checkVerdict(t, c, err)
		checkSigningInput(t, c, si)
	case "boundary_anchor_signing_input":
		...
		censusKey(rawString(t, c.Input, "public_key"))
		si, err := profile.BoundaryAnchorSigningInput(a, nil)
		checkVerdict(t, c, err)
		checkSigningInput(t, c, si)
	case "key_transition_signing_input":
		...
		si, err := profile.KeyTransitionSigningInput(kt, nil)
		checkVerdict(t, c, err)
		checkSigningInput(t, c, si)
	case "request_digest":
		got, err := profile.RequestDigest(rawString(t, c.Input, "operation"), rawValue(t, c.Input["cast_arguments"]), nil)
		checkVerdict(t, c, err)
		var want string
	...
	case "decode_grant":
		got, err := profile.DecodeGrant(rawString(t, c.Input, "compact"), nil)
		checkVerdict(t, c, err)
		if c.Expected["key_id"] != nil && got.KeyID != rawString(t, c.Expected, "key_id") {
			t.Fatalf("case %s: kid %s", c.ID, got.KeyID)
		}
	case "decode_proof":
		got, err := profile.DecodeProof(rawString(t, c.Input, "compact"), nil)
		checkVerdict(t, c, err)
	...
	case "untrusted_key_locator":
		got, err := profile.UntrustedKeyLocator(rawString(t, c.Input, "compact"), nil)
		checkVerdict(t, c, err)
	...
	case "verify_grant":
		...
		censusKey(pk)
		got, err := profile.VerifyGrant(rawString(t, c.Input, "compact"), v.TrustedIssuer{
			KeyID:     rawString(t, c.Input, "key_id"),
	...
	case "check_envelope":
		...
		censusKey(pk)
		got, err := profile.CheckEnvelope(v.Credentials{
	...
	case "encode_consumption_entry":
		got, err := profile.EncodeConsumptionEntry(v.ConsumptionEntry{
			ChainID:      rawString(t, c.Input, "chain_id"),
	...
	case "check_chain":
		...
		_, err := profile.CheckChain(v.ChainInput{Rows: rows}, v.ExpectedChain{
			ChainID:       rawString(t, c.Input, "chain_id"),
	...
	case "verify_historical_anchor":
		...
		censusKey(pk)
		_, err := profile.VerifyHistoricalAnchor(rawString(t, c.Input, "compact"), historicalKey(t, key, pk), expectedAnchor(t, subObject(t, c.Input, "expected")))
		checkVerdict(t, c, err)
	case "verify_key_transition":
		...
		_, err := profile.VerifyKeyTransition(rawString(t, c.Input, "compact"),
	...
	case "encode_anchored_export":
		...
		got, err := profile.EncodeAnchoredExport(input, expected)
		checkVerdict(t, c, err)
	...
	case "verify_anchored_export":
		...
		_, err := profile.VerifyAnchoredExport(obj, keys, expected)
		checkVerdict(t, c, err)
	default:
...
func historicalKey(t *testing.T, m map[string]json.RawMessage, pk string) v.HistoricalPublicKey {
	...
	// same mapping the Rust runner makes (ValidityUpperBound::Unbounded).
	if raw, ok := m["valid_before"]; ok && string(raw) != "null" {
		k.ValidBefore = rawInt(t, m, "valid_before")
	} else {
...
func expectedExport(t *testing.T, exp map[string]json.RawMessage) v.ExpectedAnchoredExport {
	...
	if exp["digest"] != nil {
		out.Digest = rawString(t, exp, "digest")
	}
	if exp["object_version"] != nil {
		out.ObjectVersion = rawString(t, exp, "object_version")
	}
