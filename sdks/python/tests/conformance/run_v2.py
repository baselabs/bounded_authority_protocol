#!/usr/bin/env python3
"""Conformance runner for the v2 profile of ``bounded_authority_verifier``.

Loads the published v2 corpus from ``priv/conformance/v2/corpus/``, recomputes EVERY verdict from
scratch by calling the v2 SDK façade (``bounded_authority_verifier.v2``), and asserts agreement on
all 268 cases. Asserts the ``index.json`` SHA-256 at startup (ADR 0014 D4: the SDK binds to the
exact corpus it was certified against). Runs the two-boundary key census (discovery == verify-import
...
The SDK façade was derived from spec/bap-v2.md + ADR 0004 + RFCs; this runner exercises it.
"""
...
from bounded_authority_verifier import v2  # noqa: E402
...
CORPUS_DIR = _REPO_ROOT / "priv" / "conformance" / "v2" / "corpus"
# The certified index.json SHA-256 (ADR 0014 D4). A mismatched corpus fails closed (the v2 corpus is consumed in place; the Rust/Go SDKs vendor
# snapshots — ADR 0019).
CERTIFIED_INDEX_SHA = "6de6289b7f47b0e0a78ea4610e7844a0f1d5247d8eace02ec9cf9841308f13d0"
...
def dispatch(surface: str, inp: dict[str, Any], raws: dict[str, bytes]) -> Any:
    ...
    if surface == "jwk.encode_public":
        ...
        encoded = _run_thunk(lambda: v2.jwk_encode_public(raw))
        return INVALID if encoded is INVALID else {"encoded": utf8_str(encoded)}
    ...
    if surface == "bounds.new":
        ...
        r = _run_thunk(lambda: v2.bounds_new(overrides))
    ...
    if surface == "untrusted_key_locator":
        ...
        r = _run_thunk(lambda: v2.untrusted_key_locator(compact))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {"kid": r.value.key_id}
    if surface == "grant_signing_input":
        r = _run_thunk(lambda: v2.grant_signing_input(_build_grant_producer(inp)))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {
    ...
    if surface == "proof_signing_input":
        r = _run_thunk(lambda: v2.proof_signing_input(_build_proof_producer(inp)))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {
            "protected_segment": utf8_str(r.value.protected_segment),
    ...
    if surface == "boundary_anchor_signing_input":
        r = _run_thunk(lambda: v2.boundary_anchor_signing_input(v2.BoundaryAnchorProducer(
    ...
    if surface == "key_transition_signing_input":
        r = _run_thunk(lambda: v2.key_transition_signing_input(v2.KeyTransitionProducer(
    ...
    if surface == "assemble_compact":
        ...
        r = _run_thunk(lambda: v2.assemble_compact(
            v2.SigningInput(
                kind=kind,
    ...
    if surface == "check_chain":
        ...
        r = _run_thunk(lambda: v2.check_chain(chain, expected))
        if r is INVALID or not r.is_ok:
            return INVALID
        # The v2 valid verdicts carry fact fields (the v1 corpus carried bare verdicts here).
    ...
    if surface == "verify_historical_anchor":
        ...
        r = _run_thunk(lambda: v2.verify_historical_anchor(
    ...
    if surface == "verify_key_transition":
        ...
        r = _run_thunk(lambda: v2.verify_key_transition(
    ...
    if surface == "encode_anchored_export":
        ...
        # The v2 encode cases carry no expected.digest/object_version (both are ENCODE OUTPUTS,
        # asserted in the verdict — the v1 corpus happened to carry them; the runner must not
        # require them on this surface).
        full_expected = _build_export_expected(inp, for_encode=True)
        chain = full_expected.chain
        r = _run_thunk(lambda: v2.encode_anchored_export(
            v2.AnchoredExportInput(
                rows=rows,
    ...
    if surface == "verify_anchored_export":
        ...
        r = _run_thunk(lambda: v2.verify_anchored_export(
            v2.ArchivedObject(chunks=chunks, version=version),
            v2.HistoricalKeyChain(keys=keys), expected))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {"chain_id": r.value.chain_id}
...
def _build_grant_producer(inp: dict[str, Any]) -> v2.GrantProducer:
    return v2.GrantProducer(
        ...
        operations=tuple(
            v2.OperationInput(name=op["name"], selectors=tuple(op["selectors"]))
            for op in inp.get("operations", [])
        ),
    )


def _build_proof_producer(inp: dict[str, Any]) -> v2.ProofProducer:
    cast_args = _js_to_tagged(inp["cast_arguments"])
    return v2.ProofProducer(
...
def _build_historical_key(k: dict[str, Any], ctx: str) -> v2.HistoricalPublicKey:
    vb = k.get("valid_before")
    return v2.HistoricalPublicKey(
...
def _build_expected_anchor(e: dict[str, Any], ctx: str) -> v2.ExpectedAnchor:
    return v2.ExpectedAnchor(
...
def _build_expected_transition(e: dict[str, Any], ctx: str) -> v2.ExpectedKeyTransition:
    return v2.ExpectedKeyTransition(
...
def _build_nonce(nonce: Any) -> v2.NonceNotRequired | v2.NonceRequired:
    if nonce is None:
        return v2.NonceNotRequired()
    if isinstance(nonce, dict) and isinstance(nonce.get("required"), str):
        return v2.NonceRequired(value=nonce["required"])
    abort("check_envelope: expected nonce shape")


def _build_export_expected(inp: dict[str, Any], for_encode: bool = False) -> v2.ExpectedExport:
    expected = inp.get("expected") or {}
    ...
    # The v2 corpus carries explicit (possibly empty) bounds override maps on the export surfaces;
    ...
    if isinstance(bounds_raw, dict):
        ...
        built = _run_thunk(lambda: v2.bounds_new(bounds_raw))
        if built is INVALID:
            abort("export: expected bounds")
        bounds = built
    return v2.ExpectedExport(
        chain=v2.ExpectedChain(
...
#
# v1 census delta: the v2 DECLARED census (17 entries) is an input to the index from the curated
# generator (conformance/generators/curated-inputs-v2.json `public_key_fingerprints` — facts the
# generator directory ships that are NOT derivable from case bytes), and one curated key emits no
# case: the case-byte discovery is a strict subset (16 of 17). The two-way equality leg is
# therefore curated == index (both directions), with the case-byte walk kept as the fail-closed
# subset leg (an UNDECLARED key in a case input still aborts), and the verify-import boundary leg
# unchanged from v1 (every key a valid verification case declares must be imported at the
# Ed25519 verify boundary).

_CURATED_CENSUS_PATH = _REPO_ROOT / "conformance" / "generators" / "curated-inputs-v2.json"
...
def run_census_standalone() -> None:
    ...
    print(f"census: curated-inputs-v2 == index public_key_fingerprints ({fp_count} keys, two-way); discovery ⊆ declared; verify-import ⊇ expected-verify-keys")
    if failed > 0:
        raise SystemExit(1)
