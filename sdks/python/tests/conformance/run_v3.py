#!/usr/bin/env python3
"""Conformance runner for the v3 profile of ``bounded_authority_verifier``.

Loads the published v3 corpus from ``priv/conformance/v3/corpus/`, recomputes EVERY verdict from
scratch by calling the v3 SDK façade (``bounded_authority_verifier.v3``), and asserts agreement on
all 292 cases. Asserts the ``index.json`` SHA-256 at startup (ADR 0014 D4: the SDK binds to the
exact corpus it was certified against). Runs the two-boundary key census (discovery ⊆ declared,
curated == index two-way, verify-import ⊇ expected-verify-keys) over the ES256 verify boundary.

Corpus integrity beyond the index pin: every raw sidecar's recorded SHA-256 is verified, the
revision sidecar's SHA-256 + closed shape + monotone integer revision are verified, the loaded
case count must equal the index total, and every ``tamper`` case's recorded verbatim input is
audited against the tamper-derived input (byte-for-byte re-derivation from its base case).
...
Derivation: the corpus is the published normative artifact; the runner is a consumer. The SDK façade
was derived from spec/bap-v3.md + the incorporated v1/v2 sections + RFCs; this runner exercises it.
"""
...
from bounded_authority_verifier import v3  # noqa: E402
from bounded_authority_verifier.base64url import base64url_decode, base64url_encode  # noqa: E402
from bounded_authority_verifier.ed25519 import sha256  # noqa: E402
...
from bounded_authority_verifier.v3 import imported_fingerprints, reset_census  # noqa: E402

CORPUS_DIR = _REPO_ROOT / "priv" / "conformance" / "v3" / "corpus"
# The certified index.json SHA-256 (ADR 0014 D4). A mismatched corpus fails closed (the v3 corpus is consumed in place; the Rust/Go SDKs vendor
# snapshots — ADR 0019).
CERTIFIED_INDEX_SHA = "a5c8075e7534345c3bb6611d0b40292904bcfa3af0702e07ae014fa66926433c"
...
# ---- tamper application (mirrors the corpus tamper_target_bytes discipline) ----
...
def dispatch(surface: str, inp: dict[str, Any], raws: dict[str, bytes]) -> Any:
    ...
    if surface == "untrusted_key_locator":
        ...
        r = _run_thunk(lambda: v3.untrusted_key_locator(compact))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {"kid": r.value.key_id}
    if surface == "grant_signing_input":
        r = _run_thunk(lambda: v3.grant_signing_input(_build_grant_producer(inp)))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {
    ...
    if surface == "proof_signing_input":
        r = _run_thunk(lambda: v3.proof_signing_input(_build_proof_producer(inp)))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {
            "protected_segment": utf8_str(r.value.protected_segment),
    ...
    if surface == "boundary_anchor_signing_input":
        r = _run_thunk(lambda: v3.boundary_anchor_signing_input(v3.BoundaryAnchorProducer(
    ...
    if surface == "key_transition_signing_input":
        r = _run_thunk(lambda: v3.key_transition_signing_input(v3.KeyTransitionProducer(
    ...
    if surface == "assemble_compact":
        ...
        r = _run_thunk(lambda: v3.assemble_compact(
            v3.SigningInput(
                kind=kind,
    ...
    if surface == "encode_consumption_entry":
        ...
        if r is INVALID or not r.is_ok:
            return INVALID
        # The v3 corpus encodes the canonical row bytes in the verdict as unpadded base64url
        # (the v2 corpus carried the raw JSON text — a certified per-major representation
        # choice; the runner consumes the corpus's spelling).
        return {"bytes": _b64e(r.value.bytes_), "hash": _b64e(r.value.hash_)}
    if surface == "check_chain":
        rows = tuple(byte_list(inp, "rows", "check_chain"))
        expected = v3.ExpectedChain(
        ...
        r = _run_thunk(lambda: v3.check_chain(chain, expected))
        if r is INVALID or not r.is_ok:
            return INVALID
        # The v3 valid verdicts carry fact fields (the v1 corpus carried bare verdicts here).
        return {"chain_id": r.value.chain_id}
    if surface == "request_digest":
        operation = fetch_binary(inp, "operation", "request_digest")
        cast_args = _js_to_tagged(inp["cast_arguments"])
        r = _run_thunk(lambda: v3.request_digest(operation, cast_args))
        return INVALID if (r is INVALID or not r.is_ok) else {"digest": _b64e(r.value)}
    if surface == "verify_grant":
        r = _run_thunk(lambda: v3.verify_grant(
    ...
    if surface == "verify_historical_anchor":
        ...
        r = _run_thunk(lambda: v3.verify_historical_anchor(
            str_utf8(fetch_binary(inp, "compact", "verify_historical_anchor")), key, expected))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {"anchor_id": r.value.anchor_id}
    if surface == "verify_key_transition":
        ...
        r = _run_thunk(lambda: v3.verify_key_transition(
            str_utf8(fetch_binary(inp, "compact", "verify_key_transition")), current, nxt, expected))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {"transition_id": r.value.transition_id}
    ...
    if surface == "encode_anchored_export":
        ...
        # The v3 encode cases carry no expected.digest/object_version (both are ENCODE OUTPUTS,
        # asserted in the verdict); the runner must not require them on this surface.
        full_expected = _build_export_expected(inp, for_encode=True)
        chain = full_expected.chain
        r = _run_thunk(lambda: v3.encode_anchored_export(
            v3.AnchoredExportInput(
                rows=rows,
    ...
    if surface == "verify_anchored_export":
        ...
        r = _run_thunk(lambda: v3.verify_anchored_export(
            v3.ArchivedObject(chunks=chunks, version=version),
            v3.HistoricalKeyChain(keys=keys), expected))
        if r is INVALID or not r.is_ok:
            return INVALID
        return {"chain_id": r.value.chain_id}
...
def _build_grant_producer(inp: dict[str, Any]) -> v3.GrantProducer:
    return v3.GrantProducer(
        ...
        operations=tuple(
            v3.OperationInput(name=op["name"], selectors=tuple(op["selectors"]))
            for op in inp.get("operations", [])
        ),
    )


def _build_proof_producer(inp: dict[str, Any]) -> v3.ProofProducer:
    cast_args = _js_to_tagged(inp["cast_arguments"])
    return v3.ProofProducer(
...
def _build_historical_key(k: dict[str, Any], ctx: str) -> v3.HistoricalPublicKey:
    vb = k.get("valid_before")
    return v3.HistoricalPublicKey(
...
def _build_expected_anchor(e: dict[str, Any], ctx: str) -> v3.ExpectedAnchor:
    return v3.ExpectedAnchor(
...
def _build_expected_transition(e: dict[str, Any], ctx: str) -> v3.ExpectedKeyTransition:
    return v3.ExpectedKeyTransition(
...
def _build_nonce(nonce: Any) -> v3.NonceNotRequired | v3.NonceRequired:
    if nonce is None:
        return v3.NonceNotRequired()
    if isinstance(nonce, dict) and isinstance(nonce.get("required"), str):
        return v3.NonceRequired(value=nonce["required"])
    abort("check_envelope: expected nonce shape")


def _build_export_expected(inp: dict[str, Any], for_encode: bool = False) -> v3.ExpectedExport:
    expected = inp.get("expected") or {}
    chain = expected.get("chain") or {}
    digest_raw = expected.get("digest")
    version_raw = expected.get("object_version")
    if not for_encode and (digest_raw is None or version_raw is None):
        abort("export: expected digest + object_version required at verify")
    # The v3 corpus carries explicit (possibly empty) bounds override maps on the export surfaces;
    # thread them through bounds_new so a future tightened case takes effect rather than being
    # silently dropped. Absent bounds = the struct default (maximum).
    bounds_raw = expected.get("bounds")
    bounds = None
    if isinstance(bounds_raw, dict):
        # bounds_new returns Bounds directly and raises InvalidError on a bad override map
        # (the bounds.new surface wraps the same call with _run_thunk).
        built = _run_thunk(lambda: v3.bounds_new(bounds_raw))
        if built is INVALID:
            abort("export: expected bounds")
        bounds = built
    return v3.ExpectedExport(
        chain=v3.ExpectedChain(
        ...
        # Encode placeholders: the digest is an OUTPUT of encode (asserted in the verdict), and
        # object_version is not an encode input. The encode path never reads either field.
        digest=b64_field(expected, "digest", "export") if digest_raw is not None else bytes(32),
        ...
        object_version=(
            fetch_binary(expected, "object_version", "export") if version_raw is not None else ""
        ),
        bounds=bounds,
    )
...
#
# The v3 DECLARED census is an input to the index from the curated generator
# (conformance/generators/curated-inputs-v3.json `public_key_fingerprints` — facts the generator
# directory ships that are NOT derivable from case bytes), so the case-byte discovery is checked
# as a fail-closed subset leg (an UNDECLARED key in a case input still aborts), the two-way
# equality leg is curated == index (both directions), and the verify-import boundary leg requires
# every key a valid verification case declares to be imported at the ES256 verify boundary.
# A v3 raw public key is the 65-byte uncompressed SEC1 point — 87 canonical base64url characters.

_CURATED_CENSUS_PATH = _REPO_ROOT / "conformance" / "generators" / "curated-inputs-v3.json"
...
_RAW_KEY_RE = re.compile(r"^[A-Za-z0-9_-]{87}$")
...
def _decode_census_raw_key(value: Any) -> bytes | None:
    if isinstance(value, str) and _RAW_KEY_RE.match(value):
        b = _b64d(value)
        return b if len(b) == 65 else None
    return None
...
def _collect_case_keys(value: Any, target: set[str]) -> None:
    ...
    for key, child in value.items():
        if _is_census_key_label(key):
            ...
            if raw is not None:
                target.add(_b64e(sha256(v3.thumbprint_preimage(v3.jwk_from_public_key(raw)))))
...
def run_census(index: dict[str, Any], cases: list[dict[str, Any]]) -> None:
    ...
    curated_raw = _CURATED_CENSUS_PATH.read_text("utf8")
    curated = json.loads(curated_raw)["public_key_fingerprints"]
    curated_sorted = sorted(set(curated))
    if curated != curated_sorted:
        abort("census: curated public_key_fingerprints not sorted/unique")
    declared_set = set(declared)
    curated_set = set(curated)
    if declared_set != curated_set:
        only_index = sorted(declared_set - curated_set)
        only_curated = sorted(curated_set - declared_set)
        abort(
            "census: curated != index public_key_fingerprints\n"
            f"  only-index=[{','.join(only_index)}]\n"
            f"  only-curated=[{','.join(only_curated)}]"
        )
    ...
    undeclared = sorted(discovery - declared_set)
    if undeclared:
        abort(
            "census: case inputs carry undeclared keys\n"
            f"  undeclared=[{','.join(undeclared)}]"
        )
    ...
    for fp in expected_verify:
        if fp not in verify_import:
            abort(
                ...
                "ES256 verify boundary"
            )


# ---- tamper verbatim-vs-derived audit (corpus integrity) ----


def audit_tampers(cases: list[dict[str, Any]], raws: dict[str, bytes]) -> int:
    """Re-derive every ``tamper`` case's input from its base case + tamper directive and audit it
    against the recorded VERBATIM input. The derived input is the verdict authority (run_all
    dispatches it); the audit checks the corpus guarantees the derivation actually relies on:

    - the base case exists;
    - the tamper is NON-VACUOUS (derived != base — a no-op flip would make the case a silent
      duplicate of its base);
    - the directed difference is present in the recorded verbatim input (the verbatim agrees
      with the derived value at the directed tamper target).

    The recorded verbatim input is otherwise informational: the certified v2 corpus carries one
    tamper case (verify-key-transition-v2-tamper-signature-byte) and the v3 corpus one
    (check-envelope-v3-tamper-grant-signature-byte) whose verbatim input embeds regenerated
    context beyond the directed flip — the generator sometimes re-mints surrounding fields rather
    than pure byte-flipping. Full verbatim==derived equality therefore does NOT hold on the
    certified corpora and is not the contract; the three guarantees above do."""
    ...
    for c in cases:
        ...
        if not tamper:
            continue
        base = by_id.get(tamper["base_case"])
        if base is None:
            abort(f"{c['id']}: tamper base_case {tamper['base_case']} not found")
        derived = apply_tamper(base["input"], tamper, raws)
        if derived == base["input"]:
            abort(f"{c['id']}: tamper is vacuous (derived input equals the base input)")
        # The directed target's derived value must be reflected verbatim (the target field the
        # directive names carries the directed flip in the recorded bytes).
        target = tamper.get("target")
        if target in (None, "input.text", "input.base64url"):
            field = "base64url" if target == "input.base64url" else "text"
            if derived.get(field) == base["input"].get(field):
                abort(f"{c['id']}: tamper target {target} unchanged in the derived input")
            if c["input"].get(field) == base["input"].get(field):
                abort(f"{c['id']}: tamper target {target} unchanged in the verbatim input")
        elif isinstance(target, str) and isinstance(derived.get(target), str):
            if derived[target] == base["input"].get(target):
                abort(f"{c['id']}: tamper target {target} unchanged in the derived input")
            if c["input"].get(target) == base["input"].get(target):
                abort(f"{c['id']}: tamper target {target} unchanged in the verbatim input")
        else:
            m = re.match(r"^(rows|chunks)\[(\d+)\]$", str(target))
            if m is not None:
                key = m.group(1)
                i = int(m.group(2))
                derived_list = derived.get(key)
                verbatim_list = c["input"].get(key)
                base_list = base["input"].get(key)
                if (
                    isinstance(derived_list, list) and isinstance(base_list, list)
                    and i < len(derived_list) and i < len(base_list)
                    and derived_list[i] == base_list[i]
                ):
                    abort(f"{c['id']}: tamper target {target} unchanged in the derived input")
                if (
                    isinstance(verbatim_list, list) and isinstance(base_list, list)
                    and i < len(verbatim_list) and i < len(base_list)
                    and verbatim_list[i] == base_list[i]
                ):
                    abort(f"{c['id']}: tamper target {target} unchanged in the verbatim input")
        audited += 1
    return audited
...
def run_census_standalone() -> None:
    ...
    print(f"census: curated-inputs-v3 == index public_key_fingerprints ({fp_count} keys, two-way); discovery ⊆ declared; verify-import ⊇ expected-verify-keys")
    if failed > 0:
        raise SystemExit(1)
    print("census: PASS (two-way equal + fail-closed legs)")
