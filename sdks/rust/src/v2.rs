//! Façade — the public v2 verification entry points (contract-major 2).
//!
//! The v2 profile is the frozen v1 profile carried at major 2
//! (`BAP2-Ed25519-SHA256`): every payload validator below requires `"v": 2`
//! and every domain separator is `BAP2-*` (`BAP2-REQUEST\0`,
//! `BAP2-CHAIN\0`, `BAP2-ARCHIVE\0EXPORT\0`), so v2 bytes never verify
//! under v1 and vice versa. The selector algebra alone widens: the closed kind
//! set extends to `{all, equals, one_of, lte, gte}` with same-tag inclusive
//! range semantics ([`selector::evaluate_v2`], ADR 0028). Headers, typs,
//! claims, bounds, JCS, JWK, URI rules, and facts shapes are the v1 contracts
//! unchanged.
//!
//! Derivation hygiene mirrors the v1 façade (ADR 0014 D5): derived from the
//! v1 requirement table, ADR 0028, and the certified v2 conformance corpus
//! (`conformance/corpus-v2/`) — NOT from any sibling-SDK source. `REQ1-*`
//! citations below name the v1 requirement of the same name, carried into v2
//! by the contract-major rule with only the version and domain-separator
//! deltas stated here.
//!
//! This is a **silent-auth-class surface**: a header/claim closed-set leak,
//! a v1/v2 confusion, or a single wrong signing-input byte is a wrong verdict.
//! Every reject collapses to exactly [`Invalid`](crate::Invalid) with no value
//! leak. Verification is not authority: facts carry
//! `authorization`/`trust: NotEvaluated` and there is no `allowed?` anywhere.
//!
//! The v2 profile has no local-loopback application-proof typ: the
...
// ============================================================================
// request_digest — the v2 auth-binding primitive
// ============================================================================

/// Compute the v2 request digest.
///
/// The identical typed projection and JCS preimage as the v1 primitive,
/// hashed under `BAP2-REQUEST\0` — the two majors never share a
/// request-binding domain (a v1 `ba_req` can never satisfy a v2 binding).
pub fn request_digest(
    operation: &str,
    cast_arguments: &JsonValue,
    bounds: &Bounds,
) -> Result<Vec<u8>> {
    digest::request_digest_successor(operation, cast_arguments, bounds)
}

// ============================================================================
// Constants — closed header/claim member values (suite BAP2-Ed25519-SHA256)
// ============================================================================
...
/// as `BAP2-REQUEST\0` (the v2 request digest).
///
/// `"BAP2-CHAIN\0"` = `[B, A, P, 2, -, C, H, A, I, N, 0x00]` (10 ASCII + 1 NUL
...
/// `SHA-256("BAP2-CHAIN\0" || canonical_row_bytes)`).
const CHAIN_DIGEST_PREFIX: &[u8] = b"BAP2-CHAIN\0";

/// The 20-byte archive magic prefix (ADR 0004 § Anchored export): the exact
/// ASCII bytes `BAP2-ARCHIVE\0EXPORT\0` (12 + NUL + 6 + NUL = 20). Confirmed
/// byte-exact against the corpus `anchored-export/verify.json` `chunks[0]`.
const ARCHIVE_MAGIC: &[u8] = b"BAP2-ARCHIVE\0EXPORT\0";
...
pub fn check_envelope(
    ...
    // proof's ba_req.
    let ba_req_b64u =
        digest::request_digest_successor(&expected.operation, &expected.cast_arguments, bounds)?;
...
pub fn assemble_compact(
    ...
    match input.kind {
        ...
        // The v2 profile has no local-loopback application-proof typ: the
        // byte-distinct loopback kind stays closed out (corpus
        // `assemble-compact-v2-invalid-local-loopback-kind`).
        SigningKind::LocalLoopbackHttpProof => {
            return Err(Invalid);
        }
        // The v2 profile parses no role-attestation typ: the standalone
...
pub fn proof_signing_input(proof: &ProofInput, bounds: &Bounds) -> Result<ProducedSigningInput> {
    ...
    let ba_req = digest::request_digest_successor(&proof.operation, &proof.cast_arguments, bounds)?;
...
/// `{"chain_id","commitment","previous","sequence","v":2}` (ADR 0004 §
...
/// `(canonical_bytes, SHA-256("BAP2-CHAIN\0" || canonical_bytes))`.
///
/// The corpus `entry.json` `input.previous_hash` (a base64url string) maps to
/// the row member `previous`; the producer adds `v: 2`. The corpus pins `bytes`
...
/// `{chain_id, commitment, previous, sequence, v:2}`, exact canonical bytes (a
...
/// Computes `SHA-256("BAP2-CHAIN\0" || canonical_row_bytes)` as a raw 32-byte
...
/// previous: base64url-32, sequence: int, v: 2}` — no extra members.
...
/// row_count, transition_count, v:2` (member names derived first-hand from the
/// corpus header frame).
///
...
/// v:2}`, no extra members. `chain_hash`/`key_fingerprint` are canonical
...
fn validate_anchor_payload(
    ...
    // Genesis binding (boundary_anchor_codec.ex:189): sequence 0 carries the
    // all-zero chain hash.
...
/// to_key_id, transition_id, v:2}`. Canonical form: the payload segment bytes
/// must equal the exact JCS re-encoding of the payload
...
/// row_count, transition_count, v:2}`. Enforces the canonical re-encode check
...
fn validate_grant_payload(payload: &JsonValue, bounds: &Bounds) -> Result<GrantPayload> {
    ...
    // v MUST be exactly the integer 2 (REQ1-CLAIM-v carried at major 2).
    let version = match version {
        Some(JsonValue::Int(2)) => 2,
        _ => return Err(Invalid),
    };
...
fn validate_proof_payload(payload: &JsonValue, bounds: &Bounds) -> Result<ProofPayload> {
    ...
    // v MUST be exactly the integer 2 (REQ1-CLAIM-proof-v carried at major 2).
    match version {
        Some(JsonValue::Int(2)) => {}
        _ => return Err(Invalid),
    }
...
mod tests {
    ...
    /// The vendored self-contained v2 corpus snapshot (ADR 0015 D5) — NOT the
    ...
    /// Builds a v2 range-selector grant input (the ADR 0028 shape).
    fn range_grant() -> GrantInput {
        GrantInput {
            issuer: "https://issuer.example.test".to_string(),
            grant_id: "urn:example:grant:v2-1".to_string(),
    ...
    // ==========================================================================
    // Domain separators — the BAP2-* bytes are load-bearing (major confusion is
    // the silent-auth failure this profile exists to prevent)
    // ==========================================================================

    #[test]
    fn chain_and_archive_domain_separators_carry_the_major_two_prefix() {
        // "BAP2-CHAIN\0" = 10 ASCII + 1 NUL = 11 bytes.
        assert_eq!(CHAIN_DIGEST_PREFIX, b"BAP2-CHAIN\0");
        assert_eq!(CHAIN_DIGEST_PREFIX.len(), 11);
        assert_eq!(CHAIN_DIGEST_PREFIX.last(), Some(&0u8));
        // "BAP2-ARCHIVE\0EXPORT\0" = 20 bytes (12 + NUL + 6 + NUL).
        assert_eq!(ARCHIVE_MAGIC, b"BAP2-ARCHIVE\0EXPORT\0");
        assert_eq!(ARCHIVE_MAGIC.len(), 20);
    }

    #[test]
    fn cross_major_chain_row_hashes_never_collide() {
        // Identical row fields hashed under BAP1 vs BAP2 MUST differ — a shared
        // row hash would let a v1 chain graft onto a v2 verification.
        ...
        let (v2_bytes, v2_hash) = encode_consumption_entry(&entry, &max()).expect("v2 row");
        ...
        assert_ne!(v2_bytes, v1_bytes);
        // ...but the hash differs even ignoring that: the domain separator is
        // an independent axis (manual BAP1-domain hash of the v2 row bytes).
        let mut h = sha2::Sha256::new();
        h.update(b"BAP1-CHAIN\0");
        h.update(&v2_bytes);
        let mut bap1_of_v2 = [0u8; 32];
        bap1_of_v2.copy_from_slice(&h.finalize());
        assert_ne!(v2_hash, bap1_of_v2);
        assert_ne!(v2_hash, v1_hash);
    }

    // ==========================================================================
    // Cross-major rejection — each major verifies under its own closed profile
    // only. RED if a v:2 check were widened to accept v:1 (or vice versa).
    // ==========================================================================

    #[test]
    fn each_facade_rejects_the_other_majors_bytes() {
        // Grant: the v2 producer emits v:2 bytes; v1::decode_grant rejects
        // them, v2::decode_grant accepts; the v1 producer's bytes are rejected
        // by v2::decode_grant.
        let successor_produced = grant_signing_input(&range_grant(), &max()).expect("v2 grant");
        ...
        assert!(decode_grant(successor_compact.as_bytes(), &max()).is_ok());
        assert_eq!(
            crate::v1::decode_grant(successor_compact.as_bytes(), &max()),
            Err(Invalid)
        );

        let prior_grant = GrantInput {
            issuer: "https://issuer.example.test".to_string(),
        ...
        let prior_produced =
            crate::v1::grant_signing_input(&prior_grant, &max()).expect("v1 grant");
        let prior_compact = format!(
            "{}.{}.{}",
            String::from_utf8(prior_produced.protected_segment).unwrap(),
            String::from_utf8(prior_produced.payload_segment).unwrap(),
            String::from_utf8(crate::base64url_encode(&[0u8; 64])).unwrap(),
        );
        assert_eq!(decode_grant(prior_compact.as_bytes(), &max()), Err(Invalid));
        assert!(crate::v1::decode_grant(prior_compact.as_bytes(), &max()).is_ok());
    }

    #[test]
    fn rejects_cross_major_chain_rows_both_directions() {
        let (v2_row, _) = encode_consumption_entry(
            &ConsumptionEntry {
                chain_id: "chain-x".to_string(),
                commitment: [5u8; 32],
                previous_hash: [0u8; 32],
        ...
        .expect("v2 row");
        let (v1_row, _) = crate::v1::encode_consumption_entry(
            &ConsumptionEntry {
                chain_id: "chain-x".to_string(),
                commitment: [5u8; 32],
                previous_hash: [0u8; 32],
        ...
        .expect("v1 row");
        let successor_hash_of_successor_row = {
            let mut h = sha2::Sha256::new();
        ...
        let expect_successor = ExpectedChain {
            chain_id: "chain-x".to_string(),
            first_sequence: 1,
            last_sequence: 1,
            row_count: 1,
            previous_hash: [0u8; 32],
            head_hash: successor_hash_of_successor_row,
            bounds: None,
        };
        // v2 check_chain accepts its own row, rejects the v1 row (v:1).
        assert!(check_chain(
            &ChainInput {
                rows: vec![v2_row.clone()]
            },
            &expect_successor
        )
        .is_ok());
        assert_eq!(
            check_chain(
                &ChainInput {
                    rows: vec![v1_row.clone()]
                },
                &expect_successor
            ),
            Err(Invalid)
        );
        // v1 check_chain rejects the v2 row (v:2).
        let prior_hash_of_prior_row = {
            let mut h = sha2::Sha256::new();
            h.update(b"BAP1-CHAIN\0");
            h.update(&v1_row);
            let mut out = [0u8; 32];
        ...
        let expect_prior = ExpectedChain {
            chain_id: "chain-x".to_string(),
            first_sequence: 1,
            last_sequence: 1,
            row_count: 1,
            previous_hash: [0u8; 32],
            head_hash: prior_hash_of_prior_row,
            bounds: None,
        };
        assert_eq!(
            crate::v1::check_chain(&ChainInput { rows: vec![v2_row] }, &expect_prior),
            Err(Invalid)
        );
    }

    #[test]
    fn rejects_cross_major_proof_bytes_both_directions() {
        ...
        let successor_produced = proof_signing_input(&proof, &max()).expect("v2 proof");
        let successor_compact = format!(
            "{}.{}.{}",
            String::from_utf8(successor_produced.protected_segment.clone()).unwrap(),
            String::from_utf8(successor_produced.payload_segment.clone()).unwrap(),
        ...
        assert!(decode_proof(successor_compact.as_bytes(), &max()).is_ok());
        assert_eq!(
            crate::v1::decode_proof(successor_compact.as_bytes(), &max()),
            Err(Invalid)
        );
        let prior_produced = crate::v1::proof_signing_input(&proof, &max()).expect("v1 proof");
        let prior_compact = format!(
            "{}.{}.{}",
            String::from_utf8(prior_produced.protected_segment).unwrap(),
            String::from_utf8(prior_produced.payload_segment).unwrap(),
        ...
        assert_eq!(decode_proof(prior_compact.as_bytes(), &max()), Err(Invalid));
    }

    // ==========================================================================
    // v2 request digest — BAP2-REQUEST\0, pinned by the corpus
    // ==========================================================================

    #[test]
    fn request_digest_matches_pinned_corpus_value() {
        // request-digest-v2-valid: op="read", args={limit:10,record:{id:rec-1}}
        // -> the pinned BAP2 digest (distinct from the identical v1 input's).
        ...
        assert_eq!(
            ...
            "ZLpWoq0th9geCJsvx55GoK5spV366N3pUXLLBXwkCCU"
        );
    }

    // ==========================================================================
    // lte/gte — producer byte-exactness + closed-set at the v1 façade
    // ==========================================================================

    #[test]
    fn grant_producer_emits_range_selectors_byte_exact() {
        // Corpus grant-signing-input-v2-valid-range-selectors pins all three
        // segments byte-exact — including the `"v":2` member.
        let cases = load_cases("signing-input/grant.json");
        let case = cases
            .iter()
            .find(|c| c["id"].as_str() == Some("grant-signing-input-v2-valid-range-selectors"))
            .expect("case exists");
    ...
    fn grant_producer_rejects_non_numeric_range_bound() {
        // Corpus grant-signing-input-v2-invalid-non-numeric-bound: the lte
    ...
    #[test]
    fn prior_major_grant_producer_rejects_range_selectors() {
        // The SAME range-selector grant is valid under v2 but rejected by the
        // v1 producer — the closed-set verdict flip only a contract-major
        // carries.
        assert!(grant_signing_input(&range_grant(), &max()).is_ok());
        assert_eq!(
            crate::v1::grant_signing_input(&range_grant(), &max()),
            Err(Invalid)
        );
    }
    ...
    fn decode_grant_rejects_non_numeric_bound_from_corpus() {
        // Corpus grant-decode-v2-invalid-selector-non-numeric-bound.
        let cases = load_cases("grant-decode/decode.json");
        let case = cases
            .iter()
            .find(|c| {
                c["id"].as_str() == Some("grant-decode-v2-invalid-selector-non-numeric-bound")
            })
            .expect("case exists");
    ...
    fn assemble_compact_rejects_local_loopback_kind() {
        // Corpus assemble-compact-v2-invalid-local-loopback-kind: the v2
        // profile has no loopback typ, so the kind is closed out.
    ...
    #[test]
    fn decode_grant_rejects_prior_major_bytes_from_corpus() {
        // Corpus grant-decode-v2-invalid-cross-major-v1-bytes.
        let cases = load_cases("grant-decode/decode.json");
        let case = cases
            .iter()
            .find(|c| c["id"].as_str() == Some("grant-decode-v2-invalid-cross-major-v1-bytes"))
            .expect("case exists");
        let compact = case["input"]["compact"].as_str().unwrap().as_bytes();
        assert_eq!(decode_grant(compact, &max()), Err(Invalid));
    }
