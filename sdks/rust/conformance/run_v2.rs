//! The v2 conformance runner (contract-major 2 acceptance gate).
//!
//! The v2 twin of `conformance/run.rs`, over the vendored 268-case corpus-v2
//! snapshot: an integration test living OUTSIDE `src/` so it can call ONLY the
//! crate's public surface. It:
//!
//! 1. **SHA-binds** the vendored corpus-v2 snapshot: hashes
//!    `conformance/corpus-v2/index.json` at startup and fails closed on
//!    mismatch with the certified v2 SHA.
//! 2. **Dispatches all 268 cases** across the same 28 surface names — the
//!    versioned façade functions through `bounded_authority_protocol::v2`
//!    (each executes v:2 payload validation and the BAP2 domain separators),
//!    the version-neutral primitives (`base64url`/`json`/`jcs`/`uri`/`bounds`/
//!    `jwk`) through the shared modules exactly as the v1 runner does.
//! 3. **Runs the two-boundary key census** against the v2 index's 17 declared
//!    `public_key_fingerprints` (curated-inputs-v2.json), both directions.
//! 4. Asserts `agreed == total_cases (268)` and `disagreed == 0`.
//!
//! Derivation hygiene mirrors the v1 runner (ADR 0014 §D5): derived from the
//! v1 runner, ADR 0028, and the corpus alone. The corpus is the falsifier.
...
use bounded_authority_protocol::v2;
...
const CERTIFIED_INDEX_SHA: &str =
    "6de6289b7f47b0e0a78ea4610e7844a0f1d5247d8eace02ec9cf9841308f13d0";

/// The vendored v2 corpus root (self-contained SDK test corpus — ADR 0015 D5).
...
impl KeyCensus {
    ...
    /// Records a key at its import ATTEMPT. A right-width key goes through
    /// [`KeyCensus::observe`] (the crate stays the thumbprint authority). A
    /// wrong-width key (rejected at the `[u8;32]` boundary, never fed to a
    /// crate function) is thumbprinted over the RFC 7638 preimage assembled
    /// with the crate's own JCS encoder — the v2 curated set declares the
    /// short-key case's thumbprint, so the census must observe the rejected
    /// import too. The preimage is the identical `{"crv","kty","x"}` JCS form
    /// `thumbprint_preimage` produces for accepted keys.
    fn observe_attempted(&mut self, field: &serde_json::Value) {
        ...
        self.observed
            .insert(String::from_utf8(bap::base64url_encode(&digest)).expect("base64url ASCII"));
    }
}
...
fn kind_from_str(s: &str) -> Option<SigningKind> {
    Some(match s {
        ...
        // Mapped so the corpus's closed-out loopback case REACHES the v2
        // façade (which rejects it — the v2 profile has no loopback typ);
...
fn d_request_digest(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    match v2::request_digest(operation, &cast_arguments, &max()) {
...
fn d_assemble_compact(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    match (expected_verdict, sig_is_64) {
        ...
        (_, true) => {
            ...
            match v2::assemble_compact(&input, &sig, None) {
                ...
                // The façade rejects (e.g. the v2-closed-out loopback kind or a
                // non-v2 payload) — agree iff the corpus says invalid.
                Err(Invalid) => expected_verdict == "invalid",
            }
        }
    }
...
fn d_untrusted_key_locator(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    match v2::untrusted_key_locator(compact, &max()) {
...
fn d_decode_grant(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    let expected_verdict = case["expected"]["verdict"].as_str().unwrap();
    let compact = case["input"]["compact"].as_str().unwrap().as_bytes();
    match v2::decode_grant(compact, &max()) {
...
fn d_decode_proof(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    let expected_verdict = case["expected"]["verdict"].as_str().unwrap();
    let compact = case["input"]["compact"].as_str().unwrap().as_bytes();
    match v2::decode_proof(compact, &max()) {
        Ok(d) => match expected_verdict {
...
fn d_grant_signing_input(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    match v2::grant_signing_input(&grant, &max()) {
...
fn d_proof_signing_input(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    match v2::proof_signing_input(&proof, &max()) {
        Ok(produced) => match expected_verdict {
            "valid" => {
                let exp = &case["expected"];
                produced.protected_segment == exp["protected_segment"].as_str().unwrap().as_bytes()
...
fn d_boundary_anchor_signing_input(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    // Census the key at its import attempt — the wrong-width reject included.
    ...
    match (expected_verdict, public_key_opt) {
        ("valid", Some(pk)) => {
            ...
            match v2::boundary_anchor_signing_input(&anchor, &max()) {
                Ok(produced) => {
                    let exp = &case["expected"];
        ...
        ("invalid", Some(pk)) => {
            ...
            v2::boundary_anchor_signing_input(&anchor, &max()) == Err(Invalid)
        }
        _ => false,
    }
}
...
fn d_key_transition_signing_input(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    match v2::key_transition_signing_input(&transition, &max()) {
        Ok(produced) => match expected_verdict {
            "valid" => {
                let exp = &case["expected"];
                produced.protected_segment == exp["protected_segment"].as_str().unwrap().as_bytes()
...
fn d_encode_consumption_entry(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    match v2::encode_consumption_entry(&entry, &max()) {
...
fn d_check_chain(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    match v2::check_chain(&chain_input, &expected) {
...
fn d_verify_grant(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    match v2::verify_grant(compact, &issuer, &expected) {
        Ok(facts) => match expected_verdict {
            "valid" => {
...
fn d_check_envelope(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    match v2::check_envelope(&creds, &expected) {
        Ok(facts) => match expected_verdict {
            "valid" => {
                facts.authorization == NotEvaluated
                    && facts.grant.authorization == NotEvaluated
                    && facts.grant.version == 2
            }
            _ => false,
        },
        Err(Invalid) => expected_verdict == "invalid",
...
fn d_verify_historical_anchor(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    let compact = input["compact"].as_str().unwrap().as_bytes();
    match v2::verify_historical_anchor(compact, &key, &expected) {
        Ok(facts) => match expected_verdict {
            "valid" => {
...
fn d_verify_key_transition(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    let compact = input["compact"].as_str().unwrap().as_bytes();
    match v2::verify_key_transition(compact, &current, &next, &expected) {
        Ok(facts) => match expected_verdict {
            "valid" => {
...
fn d_encode_anchored_export(case: &serde_json::Value, _census: &mut KeyCensus) -> bool {
    ...
    let expected = ExpectedExport {
        ...
        transitions: input["expected"]["transitions"]
            ...
            .map(expected_transition_from)
            .collect(),
        // The v2 encode cases omit object_version/digest (the producer never
        // consults them; the v1 corpus carried inert mirrors) — default when
        // absent.
    ...
    match v2::encode_anchored_export(&anchored_input, &expected) {
...
fn d_verify_anchored_export(case: &serde_json::Value, census: &mut KeyCensus) -> bool {
    ...
    match v2::verify_anchored_export(&obj, &keys, &expected) {
        Ok(facts) => match expected_verdict {
            "valid" => {
                facts.trust == NotEvaluated
                    && facts.authorization == NotEvaluated
...
fn conformance_full_corpus() {
    ...
    for entry in index["files"].as_array().expect("files array") {
        ...
        if path == "revision.json" {
            ...
            assert_eq!(
                ...
                Some("bounded-authority-protocol-v2-conformance-corpus-revision"),
    ...
    assert_eq!(agreed, total_cases, "agreed == total_cases (268)");
