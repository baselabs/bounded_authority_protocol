defmodule BoundedAuthorityProtocol.V2.KeyTransitionTest do
  ...
  alias BoundedAuthorityProtocol.V2.KeyTransitionCodec
  alias BoundedAuthorityProtocol.V2.KeyTransitionFacts

  test "produces, assembles, and verifies a v2 authenticated standard-JWS key transition" do
    {current_public, current_private} = ed25519_keypair(<<2::256>>)
    {next_public, _next_private} = ed25519_keypair(<<3::256>>)
    transition = transition(current_public, next_public)

    assert {:ok, signing_input} = V2.key_transition_signing_input(transition, %{})
    ...
    assert decode(signing_input.payload_segment) ==
             ~s({"chain_id":"urn:example:chain","effective_at":2000,"from_key_fingerprint":"#{b64(current_fingerprint)}","to_key_fingerprint":"#{b64(next_fingerprint)}","to_key_id":"archive-key-b","transition_id":"urn:example:transition:a-b","v":2})
    ...
    assert inspect(facts) == "#BoundedAuthorityProtocol.V2.KeyTransitionFacts<redacted>"
  end

  test "a v1-signed transition is rejected by v2 verify and parse" do
    {current_public, current_private} = ed25519_keypair(<<2::256>>)
    {next_public, _next_private} = ed25519_keypair(<<3::256>>)
    {:ok, current_fingerprint} = Jwk.public_key_thumbprint_raw(current_public, %{})
    {:ok, next_fingerprint} = Jwk.public_key_thumbprint_raw(next_public, %{})
    ...
    {:ok, v2_input} =
      V2.key_transition_signing_input(transition(current_public, next_public), %{})

    {:ok, v1_input} =
      V1.key_transition_signing_input(transition(current_public, next_public), %{})

    refute v2_input.message == v1_input.message

    signature = :crypto.sign(:eddsa, :none, v1_input.message, [current_private, :ed25519])
    ...
    assert {:error, :invalid} = KeyTransitionCodec.parse(v1_compact, Bounds.maximum())
  end
  ...
  test "rejects unchanged keys, bad adjacent windows, wrong kind, fields, and signature sizes" do
    ...
    assert {:error, :invalid} =
             V2.key_transition_signing_input(transition(current_public, current_public), %{})

    {:ok, input} = V2.key_transition_signing_input(transition(current_public, next_public), %{})
    ...
    assert {:error, :invalid} =
             V2.key_transition_signing_input(
    ...
    assert {:error, :invalid} = V2.assemble_compact(%{input | kind: :boundary_anchor}, signature)
    assert {:error, :invalid} = V2.assemble_compact(input, binary_part(signature, 0, 63))
    assert {:error, :invalid} = V2.assemble_compact(input, signature <> <<0>>)
