defmodule BoundedAuthorityProtocol.V2.ConsumptionChainTest do
  ...
  alias BoundedAuthorityProtocol.V2.ChainFacts
  alias BoundedAuthorityProtocol.V2.ConsumptionChain
  ...
  test "encodes exact canonical v2 rows with domain-separated hashes" do
    ...
    assert {:ok, encoded} = V2.encode_consumption_entry(entry, %{})

    assert encoded.bytes ==
             ~s({"chain_id":"urn:example:chain","commitment":"#{b64(@commitment_a)}","previous":"#{b64(@zero_hash)}","sequence":1,"v":2})

    assert encoded.hash == :crypto.hash(:sha256, "BAP2-CHAIN\0" <> encoded.bytes)
    refute encoded.hash == :crypto.hash(:sha256, encoded.bytes)
  end

  test "parse_row round-trips exact canonical rows and their hashes" do
    {:ok, encoded} = V2.encode_consumption_entry(entry(1, @zero_hash, @commitment_a), %{})
  ...
  test "legacy-major rows are rejected by v2 parse_row and check_chain" do
    ...
    assert {:ok, v2_first} = V2.encode_consumption_entry(entry(1, @zero_hash, @commitment_a), %{})
  ...
  test "checks genesis and continued ranges only against mandatory caller boundaries" do
    {:ok, first} = V2.encode_consumption_entry(entry(1, @zero_hash, @commitment_a), %{})
    {:ok, second} = V2.encode_consumption_entry(entry(2, first.hash, @commitment_b), %{})
    ...
    assert {:ok,
            %ChainFacts{
              ...
              trust: :not_evaluated
            } = facts} = V2.check_chain(%ChainInput{rows: [first.bytes, second.bytes]}, expected)

    assert inspect(facts) == "#BoundedAuthorityProtocol.V2.ChainFacts<redacted>"

    continued = expected_chain(2, 2, 1, first.hash, second_hash)

    assert {:ok, %ChainFacts{first_sequence: 2}} =
             V2.check_chain(%ChainInput{rows: [second.bytes]}, continued)
  end
  ...
  test "rejects noncanonical, unclosed, and malformed rows" do
    ...
    reordered =
      ~s({"v":2,"sequence":1,"previous":"#{b64(@zero_hash)}","commitment":"#{b64(@commitment_a)}","chain_id":"urn:example:chain"})

    # The closed row shape admits exactly the five v2 keys.
    unclosed =
      ~s({"chain_id":"urn:example:chain","commitment":"#{b64(@commitment_a)}","extra":"x","previous":"#{b64(@zero_hash)}","sequence":1,"v":2})
    ...
    for input <- [
      ...
      assert {:error, :invalid} = V2.check_chain(input, expected)
    end
  ...
  test "empty and over-limit row sets fail independently" do
    ...
    assert {:error, :invalid} =
             V2.check_chain(%ChainInput{rows: [first.bytes, second.bytes]}, bounded)
  end

  test "sequence, chain-id, and predecessor link checks fail independently" do
    {:ok, first} = V2.encode_consumption_entry(entry(1, @zero_hash, @commitment_a), %{})
    {:ok, wrong_link} = V2.encode_consumption_entry(entry(2, @zero_hash, @commitment_b), %{})
    {:ok, wrong_sequence} = V2.encode_consumption_entry(entry(3, first.hash, @commitment_b), %{})
    {:ok, wrong_chain} = V2.encode_consumption_entry(other_chain_entry(2, first.hash), %{})
  ...
  test "encode rejects genesis-link violations, row-byte bounds, and bad identifiers" do
    assert {:error, :invalid} =
             V2.encode_consumption_entry(entry(1, <<1::256>>, @commitment_a), %{})

    assert {:error, :invalid} =
             V2.encode_consumption_entry(entry(2, @zero_hash, @commitment_b), %{
               chain_row_bytes: 1
             })

    assert {:error, :invalid} =
             V2.encode_consumption_entry(
