defmodule BoundedAuthorityProtocol.V3.AnchoredExportTest do
  ...
  alias BoundedAuthorityProtocol.V3.AnchoredExportCodec
  alias BoundedAuthorityProtocol.V3.AnchoredExportFacts
  alias BoundedAuthorityProtocol.V3.EcJwk

  @zero_hash <<0::256>>
  # The v2 archive prefix is byte-distinct from the v1 BAP1 prefix at frame offset 0.
  @archive_prefix "BAP3-ARCHIVE\0EXPORT\0"

  test "atomically verifies a chunk-split v3 export across an authenticated key rollover" do
    context = rollover_context()

    assert {:ok, encoded} = V3.encode_anchored_export(context.input, context.expected_export)
    ...
    assert {:ok,
            %AnchoredExportFacts{
             ...
             V3.verify_anchored_export(archived, context.key_chain, expected)

    assert inspect(facts) == "#BoundedAuthorityProtocol.V3.AnchoredExportFacts<redacted>"
  end

  test "a legacy-major export with matching context is rejected by v3 verification" do
  ...
  test "rejects digest/version/EOF/frame/transition/coverage drift and non-raw evidence" do
    context = rollover_context()
    {:ok, encoded} = V3.encode_anchored_export(context.input, context.expected_export)
    ...
    assert {:error, :invalid} =
             V3.verify_anchored_export(archived, context.key_chain, %{expected | digest: <<0>>})
    ...
    assert {:error, :invalid} = V3.verify_anchored_export(encoded, context.key_chain, expected)

    {:ok, facts} = V3.check_chain(%ChainInput{rows: context.input.rows}, context.chain)

    assert {:error, :invalid} = V3.verify_anchored_export(facts, context.key_chain, expected)
  ...
  test "same-key boundaries require no transition and permit equal anchor times" do
    {public_key, private_key} = p256_keypair(<<6::256>>)
    ...
    assert {:ok, encoded} = V3.encode_anchored_export(input, export_expected)
    ...
    assert {:ok, %AnchoredExportFacts{version: 3, transition_count: 0}} =
             V3.verify_anchored_export(
  ...
  test "rejects non-increasing transition order and broken expected-key continuity" do
    ...
    {unrelated_public, _unrelated_private} = p256_keypair(<<7::256>>)
    {:ok, unrelated_fingerprint} = EcJwk.public_key_thumbprint_raw(unrelated_public, %{})
  ...
  defp rollover_context do
    {current_public, current_private} = p256_keypair(<<4::256>>)
    {next_public, next_private} = p256_keypair(<<5::256>>)
  ...
  defp legacy_major_rollover_context do
    {current_public, current_private} = legacy_ed25519_keypair(<<4::256>>)
    {next_public, next_private} = legacy_ed25519_keypair(<<5::256>>)
  ...
  defp rows_and_chain do
    ...
    {:ok, first} = V3.encode_consumption_entry(first_entry, %{})
    ...
    {:ok, second} = V3.encode_consumption_entry(second_entry, %{})
  ...
  defp sign_anchor(anchor, private_key) do
    {:ok, input} = V3.boundary_anchor_signing_input(anchor, %{})
  ...
  defp sign_transition(transition, private_key) do
    {:ok, input} = V3.key_transition_signing_input(transition, %{})
    signature = sign_es256(input.message, private_key)
    {:ok, compact} = V3.assemble_compact(input, signature)
    compact
  end
