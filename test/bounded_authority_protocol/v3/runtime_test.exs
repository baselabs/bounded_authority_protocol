defmodule BoundedAuthorityProtocol.V3.RuntimeTest do
  ...
  test "grant producer emits v3 payload bytes with range selectors" do
    {:ok, input} = V3.grant_signing_input(grant(), %{})

    protected = decode_segment(input.protected_segment)
    payload = decode_segment(input.payload_segment)

    assert %{"v" => 3} = payload
    assert %{"typ" => "ba+cap", "alg" => "ES256", "kid" => @issuer_key_id} = protected
    ...
    assert {:ok, %BoundedAuthorityProtocol.V3.DecodedGrant{} = decoded} =
             V3.decode_grant(compact, %{})
  ...
  test "producer rejects non-numeric and malformed range bounds" do
    for selectors <- [
      ...
      grant = %{
        grant()
        | operations: [
            %BoundedAuthorityProtocol.V3.Operation{name: "transfer", selectors: selectors}
          ]
      }

      assert {:error, :invalid} = V3.grant_signing_input(grant, %{}), inspect(selectors)
    end
  end
  ...
  test "decode rejects non-numeric range bounds and unrecognized kinds on the wire" do
    for selector <- [
      ...
      assert {:error, :invalid} = V3.decode_grant(compact, %{}), inspect(selector)
    end
  end

  test "verify_grant accepts a correctly signed v3 grant and returns v3 facts" do
    {compact, public} = signed_grant()

    assert {:ok, facts} = V3.verify_grant(compact, trusted(public), expected_grant())
    ...
    assert inspect(facts) == "#BoundedAuthorityProtocol.V3.GrantFacts<redacted>"

    assert {:ok, issuer_fingerprint} =
             EcJwk.public_key_thumbprint_raw(public, %{})
  ...
  test "verify_grant rejects v1-major bytes (no cross-major fallback)" do
    ...
    assert {:error, :invalid} =
             V3.verify_grant(v1_compact, trusted(legacy_major_key()), expected_grant())

    assert {:error, :invalid} = V3.decode_grant(v1_compact, %{})
  end
  ...
  test "verify_grant rejects the v1 closed rejection classes under v2 constants" do
    ...
    for {trusted, expected} <- [
      ...
      assert {:error, :invalid} = V3.verify_grant(compact, trusted, expected), inspect(expected)
    end

    assert {:error, :invalid} =
             V3.verify_grant(flip_segment(compact, 2), trusted(public), expected_grant())
  end
  ...
  test "check_envelope accepts interval-bounded arguments and rejects out-of-range ones" do
    for {amount, verdict} <- [
      ...
      case verdict do
        :ok ->
          assert {:ok, %BoundedAuthorityProtocol.V3.EnvelopeFacts{}} =
                   V3.check_envelope(credentials, expected)

        :error ->
          assert {:error, :invalid} = V3.check_envelope(credentials, expected)
      end
    end
  end
  ...
  test "check_envelope binds floats on the float tag and signed zero" do
    ...
    assert {:ok, _facts} = V3.check_envelope(credentials, expected)
  ...
  test "check_envelope rejects tampered segments and mismatched context" do
    ...
    for invalid <- [
      ...
      assert {:error, :invalid} = V3.check_envelope(invalid, expected)
    end
  end

  test "check_envelope returns v3 redacted non-authorizing facts" do
    {credentials, expected} = envelope(75)

    assert {:ok, facts} = V3.check_envelope(credentials, expected)
    ...
    assert inspect(facts) == "#BoundedAuthorityProtocol.V3.EnvelopeFacts<redacted>"
  end

  test "assemble_compact validates the assembled v3 artifact and rejects unknown kinds" do
    {compact, _public} = signed_grant()

    assert {:ok, %BoundedAuthorityProtocol.V1.KeyLocator{trust: :not_evaluated}} =
             V3.untrusted_key_locator(compact, %{})

    {:ok, input} = V3.grant_signing_input(grant(), %{})
    signature = sign_es256(input.message, issuer_private())
    assert {:ok, assembled} = V3.assemble_compact(input, signature)
    assert {:ok, decoded} = V3.decode_grant(assembled, %{})
    assert decoded.version == 3

    assert {:error, :invalid} = V3.assemble_compact(:not_an_input, signature, %{})
    assert {:error, :invalid} = V3.assemble_compact(input, <<0::504>>, %{})
    ...
    assert {:error, :invalid} = V3.assemble_compact(local_loopback_kind, signature, %{})
  end

  test "every codec delegation arm fails closed on malformed inputs" do
    for {result} <- [
          {V3.encode_consumption_entry(:not_an_entry, %{})},
          {V3.check_chain(:not_an_input, :not_expected)},
          {V3.boundary_anchor_signing_input(:not_an_anchor, %{})},
          {V3.key_transition_signing_input(:not_a_transition, %{})},
          {V3.encode_anchored_export(:not_an_input, :not_expected)},
          {V3.verify_anchored_export(:not_archived, :not_keys, :not_expected)}
        ] do
      assert {:error, :invalid} = result
    end

    assert {:error, :invalid} =
             V3.verify_historical_anchor(:not_binary, :not_a_key, :not_expected)

    assert {:error, :invalid} =
             V3.verify_key_transition(:not_binary, :not_a_key, :not_a_key, :not_expected)
  end
  ...
  test "producer and decode error arms for the inherited kinds and fields fail closed" do
    ...
    for operations <- [
      ...
      assert {:error, :invalid} = V3.grant_signing_input(grant, %{}), inspect(operations)
    end
    ...
    assert {:error, :invalid} = V3.decode_grant(compact, %{})

    # decode_audiences: scalar string, list with invalid member, and non-string shapes
    for aud <- [%{}] do
      payload = forged_payload_with("aud", aud)
      assert {:error, :invalid} = V3.decode_grant(forged_grant_with_payload(payload), %{})
    end

    # audience scalar form accepts; invalid scalar rejected
    assert {:error, :invalid} =
             V3.decode_grant(forged_grant_with_payload(forged_payload_with("aud", "")), %{})
    ...
    for invalid <- [
      ...
      assert {:error, :invalid} = V3.check_envelope(credentials, invalid)
    end

    # valid_string_list? catch-all via the producer
    bad = %{grant() | audiences: :not_a_list}
    assert {:error, :invalid} = V3.grant_signing_input(bad, %{})

    # optional_nonce invalid-typed member
    payload = forged_payload_with("nonce", 5)
    assert {:error, :invalid} = V3.decode_proof(forged_proof_with_payload(payload), %{})
  end
  ...
  test "inherited-kind producer success and decode error arms close the remaining paths" do
    ...
    assert {:ok, input} = V3.grant_signing_input(mixed, %{})
    ...
    {:ok, input2} = V3.grant_signing_input(grant(), %{})
    ...
    signature = sign_es256(message, issuer_private())

    assert {:error, :invalid} =
             V3.decode_grant(message <> "." <> Base.url_encode64(signature, padding: false), %{})
    ...
    signature2 = sign_es256(message2, issuer_private())
    ...
    assert {:error, :invalid} =
             V3.decode_proof(forged_proof_with_payload(forged_proof_member("nonce", true)), %{})
    ...
    assert {:ok, _facts} = V3.check_envelope(credentials, expected)
    assert {:error, :invalid} = V3.check_envelope(credentials, %{expected | nonce: :not_required})
  end

  test "improper selector containers raise through the fixed guard and fail closed" do
    op = %BoundedAuthorityProtocol.V3.Operation{name: "transfer", selectors: [:all]}
    grant = %{grant() | operations: [op | :invalid_tail]}

    assert {:error, :invalid} = V3.grant_signing_input(grant, %{})
  end
  ...
  test "assembly revalidation fails closed on a well-formed signing input with invalid payload members" do
    ...
    assert {:error, :invalid} = V3.assemble_compact(forged_grant, <<0::512>>, %{})
    ...
    proof = %BoundedAuthorityProtocol.V3.Proof{
    ...
    {:ok, proof_input} = V3.proof_signing_input(proof, %{})
    ...
    assert {:error, :invalid} = V3.assemble_compact(forged_proof, <<0::512>>, %{})
  end
  ...
  test "a mixed-kind grant decodes its one_of selector through the v2 parse path" do
    ...
    {:ok, input} = V3.grant_signing_input(mixed, %{})
    signature = sign_es256(input.message, issuer_private())
    {:ok, compact} = V3.assemble_compact(input, signature)

    assert {:ok, decoded} = V3.decode_grant(compact, %{})
  ...
  test "malformed runtime inputs return the fixed error" do
    assert {:error, :invalid} = V3.decode_grant(:not_binary, %{})
    assert {:error, :invalid} = V3.decode_proof(:not_binary, %{})
    assert {:error, :invalid} = V3.grant_signing_input(:not_a_grant, %{})
    assert {:error, :invalid} = V3.proof_signing_input(:not_a_proof, %{})

    assert {:error, :invalid} =
             V3.verify_grant(:not_binary, trusted(legacy_major_key()), expected_grant())

    assert {:error, :invalid} = V3.check_envelope(%{}, %{})
    assert {:error, :invalid} = V3.verify_grant("a.b.c", :not_trusted, expected_grant())
  end
  ...
  defp grant do
    %BoundedAuthorityProtocol.V3.Grant{
  ...
  defp forged_grant_with_selector(selector) do
    grant = %{
      grant()
      | operations: [
          %BoundedAuthorityProtocol.V3.Operation{name: "transfer", selectors: [:all]}
        ]
    }
  ...
  defp issuer_public, do: elem(:crypto.generate_key(:ecdh, :prime256v1, <<1::256>>), 0)
  defp issuer_private, do: elem(:crypto.generate_key(:ecdh, :prime256v1, <<1::256>>), 1)
  defp holder_public, do: elem(:crypto.generate_key(:ecdh, :prime256v1, <<2::256>>), 0)
  defp holder_private, do: elem(:crypto.generate_key(:ecdh, :prime256v1, <<2::256>>), 1)

  defp holder_thumbprint do
    {:ok, jwk_bytes} = EcJwk.encode_public(holder_public(), %{})
    {:ok, raw} = EcJwk.thumbprint_raw(jwk_bytes, %{})
    raw
  end
  ...
  defp credentials_and_expected(amount, grant \\ grant()) do
    ...
    {:ok, input} = V3.proof_signing_input(proof, %{})
    signature = sign_es256(input.message, holder_private())
    {:ok, proof_compact} = V3.assemble_compact(input, signature)
  ...
  defp forged_proof_with_payload(payload) do
    ...
    payload_segment = Base.url_encode64(json_bytes(payload), padding: false)
    message = input.protected_segment <> "." <> payload_segment
    signature = sign_es256(message, holder_private())
    message <> "." <> Base.url_encode64(signature, padding: false)
  end
