defmodule BoundedAuthorityProtocol.V3.RequestDigestTest do
  use ExUnit.Case, async: true

  alias BoundedAuthorityProtocol.V1
  alias BoundedAuthorityProtocol.V3, as: V3
  alias BoundedAuthorityProtocol.V3.RequestDigest

  # The v2 profile hashes the identical canonical argument projection under the
  # BAP3-REQUEST domain separator, so every v2 digest must differ from v1's.
  ...
  test "digest is domain-separated from v1 over identical canonical bytes" do
    ...
    assert {:ok, v2_digest} = V3.request_digest("read_record", arguments, %{})
    assert {:ok, v1_digest} = V1.request_digest("read_record", arguments, %{})
    refute v2_digest == v1_digest

    expected =
      :crypto.hash(:sha256, ["BAP3-REQUEST\0", @canonical])
  ...
  test "all tagged JSON root shapes are valid request arguments" do
    for {arguments, canonical} <- [
      ...
      assert {:ok, ^expected} = V3.request_digest("read_record", arguments, %{})
    end

    assert {:ok, integral_float} = V3.request_digest("read_record", {:float, 1.0}, %{})

    expected_integral_float =
      :crypto.hash(:sha256, ["BAP3-REQUEST\0", ~s(["read_record",["float",1]])])
      |> Base.url_encode64(padding: false)
  ...
  test "typed projection preserves the distinction between integer and float tags" do
    assert {:ok, integer_digest} = V3.request_digest("op", {:integer, 1}, %{})
    assert {:ok, float_digest} = V3.request_digest("op", {:float, 1.0}, %{})

    refute integer_digest == float_digest

    assert {:ok, nested_integer} =
             V3.request_digest("op", {:array, [{:integer, 1}]}, %{})

    assert {:ok, nested_float} = V3.request_digest("op", {:array, [{:float, 1.0}]}, %{})
  ...
  test "object member order is semantic while arrays and scalar tags remain exact" do
    ...
    assert V3.request_digest("op", left, %{}) == V3.request_digest("op", right, %{})
    refute V3.request_digest("op", left, %{}) == V3.request_digest("op", reordered_array, %{})
  end

  test "operation-name bound accepts exactly 128 bytes and rejects maximum plus one" do
    assert {:ok, _digest} = V3.request_digest(String.duplicate("a", 128), :null, %{})
    assert {:error, :invalid} = V3.request_digest(String.duplicate("a", 129), :null, %{})
    assert {:error, :invalid} = V3.request_digest("line\nbreak", :null, %{})
  end

  test "malformed values and widening bounds always return the fixed error" do
    assert {:error, :invalid} = V3.request_digest(:operation, :null, %{})
    assert {:error, :invalid} = V3.request_digest("op", nil, %{})
    assert {:error, :invalid} = V3.request_digest("op", {:integer, :not_an_integer}, %{})
    assert {:error, :invalid} = V3.request_digest("op", :null, %{jcs_bytes: 65_537})
    assert {:error, :invalid} = V3.request_digest("op", :null, %{unknown: 1})

    assert {:error, :invalid} =
             V3.request_digest("op", {:array, [{:integer, 1} | :invalid_tail]}, %{})

    assert {:error, :invalid} =
             V3.request_digest("op", {:array, [:invalid_member]}, %{})
    ...
    assert {:error, :invalid} = V3.request_digest("op", {:object, [{1, :null}]}, %{})
  end

  test "digest_raw returns the raw 32-byte digest and the fixed error on failure" do
    assert {:ok, encoded} = V3.request_digest("read_record", {:integer, 1}, %{})
    assert {:ok, raw} = RequestDigest.digest_raw("read_record", {:integer, 1}, %{})

    assert byte_size(raw) == 32
    assert raw == :crypto.hash(:sha256, ["BAP3-REQUEST\0", ~s(["read_record",["integer",1]])])
