...
# The guard under test is a POSIX shell script driven through `sh -c` with stdin
# redirection; both are unavailable on Windows, where the guard is POSIX-only
# tooling. The module and its tests are defined only on POSIX platforms.
unless BoundedAuthorityProtocol.TestSupport.Portable.windows?() do
