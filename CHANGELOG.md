...
- Close the BAP-15 prerequisite spec/corpus gaps (no-key half). `docs/protocol-v1.md` gains a
  ...
  §6.1.6.1.20 cited). The conformance corpus grows **259 → 279 cases**: 5 JCS float cases pinning
  ...
  nonzero chain_hash), and `verify_anchored_export` (empty chunk). Every invalid case is a
  one-defect **skip-would-accept** construction (a verifier that drops its target check would accept
  it) — proven by defect-injecting each target check in the independent runner and confirming the
  case flips. The two `encode_/verify_anchored_export.maximum_plus_one` `n_a` reasons are corrected
  to name all three bounds (`archive_bytes` + `archive_chunks` + `historical_key_transitions`) and
  the loader representation constraint. The independent Node runner is strengthened to mirror the
  reference's IPv6-structure (node:net `isIP`, replacing a crude charset check), consumption-entry
  and boundary-anchor genesis bindings, `check_chain` positive-`first_sequence` and canonical-rehash
  ...
  cases exposed; every new case is dual-verified (`agreed=279 disagreed=0`). SDK
  `CERTIFIED_INDEX_SHA` constants (Python hex + TypeScript base64url) rebind to the new `index.json`.
  The signed anchored-export cases (`#2` non-monotone chronology, `#2` fingerprint cycle, `#3`
  one-key valid — three seeded Ed25519 keys) **and the `encode_anchored_export` start-anchor-binding
  case** (which needs new signed anchors with isolate-able sequences) remain as a follow-up.
...
- Ship cross-language verifier SDKs (TypeScript `@bounded-authority/verifier` + Python
  ...
  ([ADR 0014](docs/adr/0014-cross-language-verifier-sdks.md)). Each passes all 279 conformance vectors
