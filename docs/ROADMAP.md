...
| BAP-23 | **Role-attestation sibling profile — `bap-role-attestation/1`** — The BA-signed role attestation as the protocol's first sibling attestation profile (ADR 0036): a standalone, grant-unbound compact binding a subject key to `issuer`/`holder` for a bounded window, verified by `BoundedAuthorityProtocol.RoleAttestation.V1.verify_attestation/2` — the BARA RA11 prerequisite crossing the dependency wall as data, slug:bap-23-role-attestation | [ADR 0036](adr/0036-role-attestation-profile.md) + [`spec/bap-role-attestation-v1.md`](../spec/bap-role-attestation-v1.md) + [role-attestation requirement map](design/role-attestation-requirement-map.md) land together; the profile corpus (40 cases, revision 1, certified index `be5275c6…d62a`, pinned in the ExUnit suite, the requirement map, the spec, and every SDK) verifies 40/40 decode+verify with v1/v2/v3 cross-rejection both directions through `mix role_attestation.verify`; ten mutation-battery entries (self-attestation ×2, containment ×2, now-window, kid binding, subject binding, role closed set, canonical bytes, signature) redden the corpus test one-match each; `ba+role-attestation` active in registries with the attestation-profiles family; the governance class, the standards-track evolution contract, and ADR 0007's requirement ranges carry the labeled ADR-0036 amendments; the v1/v2 `parse` closed-error-shape defect (bare `false` on wrong-width signatures, found by the profile's ES256 corpus case) is fixed RED-first with `signature_width_error_shape_test.exs`; the in-repo Python/Rust/Go SDKs implement the four surfaces; v1/v2/v3 corpora and certified pins unchanged (labeled `SigningInput`/`CompactJws` kind touches only); **release preconditions** (any `0.x.0` bearing this): the TS verifier surface coordinated at corpus freeze, the authority-runtime issuance receipt, and the companion-signer consumption receipt in their own repositories — release itself is the owner's decision; **cross-vendor review** executed 2026-09-22 over the full landing range (639d74e..e5cd033): the claude-family lens reviewed with receipts (4 findings, no blockers — all adjudicated, the two should-fix defects and a decode-surface alignment fixed in-repo: attestor-window magnitude parity in the Python/Go SDKs and the certified index pin in the `role_attestation.verify` script; corpus and certified pins unchanged), the codex-family lens completed September 23, 2026 over the inclusive landing and parity-fix range (`639d74e..d470814`), confirming eight product findings (three blocking, five should-fix) and one harness note; the product findings have RED-first repairs in Elixir and the three in-repo SDKs, with the harness note recorded separately; the terminal repair review confirms all eight fixes, and final `mix quality` completed with exit 0 (584 passed, 100% coverage); dated receipts and adjudications live under `.kimosabe/reviews/`; see the Codex review evidence below | BAP-22 | [ADR 0036](adr/0036-role-attestation-profile.md), [ADR 0027](adr/0027-byte-distinct-application-proof-profiles.md), [ADR 0029](adr/0029-budget-window-posture.md), and the [successor-major charter](design/successor-major-charter.md) |
...
### BAP-07 closeout backfill (additive, 2026-08-26 — the spec-decoupling program)

The gate-integrity review's deferred follow-up "a corpus index-SHA pin in the Elixir
`conformance.verify` stage" closed twice over: the pin landed in the 0.1.2 hardening
(`1cb768f`), and the spec-decoupling program then made the rotation itself a gate — six
machine-checked certified-index-SHA pins (four SDK runners in their native encodings plus the
Elixir CLI and its test mirror) regenerated by one command
(`scripts/regen_corpus_digests.exs`) with a `mix quality` leg that reds on any drifted
constant. The corpus also gained the tamper-covered `revision.json` revision sidecar
(`e769458`), closing the "cited format string could detect nothing" gap: corpus-dependent
documents cite the sidecar's monotone integer, gated against drift.

### BAP-18 closeout backfill (additive, 2026-08-26 — the spec-decoupling program)

The bounds-aware `assemble_compact/3` posture's documentation caught up to the authority
swap: the normative v1 wire profile now lives in `spec/bap-v1.md` (single authority; the
frozen façade including both arities is its informative Appendix C), `docs/protocol-v1.md` is
a generated derived view (regeneration-diff gated), and every bounds/verdict statement is
machine-extracted from the spec and gated against the live `Bounds.maximum/0` dump (rule 1),
...
## BAP-23 Codex review evidence (September 23, 2026)

- OBSERVED: the native Codex review completed with exit 0 over `639d74e..d470814`
  (`e5cd033^..d470814`, including the profile landing and parity fixes). The two earlier
  quota skips remain in the historical record. The September 22 Claude receipt stands.
  The repaired implementation is committed as `05c24a8`.
- OBSERVED in source inspection and executed regressions: eight product findings cover
  malformed caller inputs, closed member sets, identifier/URI validation, caller-tightened
  bounds, producer/consumer symmetry, and bounds checked before decoding. Repairs remain
  within the role-attestation implementations and tests; the architecture gate retains an
  exact compiler-call census, adjusted for four added producer validation steps.
- OBSERVED: Python's final suite passed 171 tests; Rust's passed 502; Go's full SDK gate and
  subsequent affected-package URI checks passed. Real signed URI cases match the existing
  Elixir StringOrUri implementation. Parser work ordering is established by source inspection
  and tested size-projection arithmetic; allocator behavior was not measured.
- OBSERVED: all 121 tracked corpus/ADR-0036 files match `d470814` byte-for-byte; the certified
  role-attestation index remains `be5275c69539a0f31734242ff00a484c2f855f39181c55689d8b0f671195d62a`.
  The corpus harness's boolean verdict checks do not independently prove exact invalid-result
  shape; this harness-only note is recorded separately from the repaired product findings.
- OBSERVED: the terminal Claude-family repair review completed with exit 0 and confirmed all
  eight named fixes, with no blocking findings. Its three documentation/evidence notes are
  resolved by changelog wording, additional reference probes, and hash-pinned supervisor tests.
  Final `mix quality` completed with exit 0: `584 passed (14 properties, 570 tests)`,
  100% line coverage, 47 chain/archive mutations, 82 conformance mutations, and 16 spec-facts
  mutations caught; the role-attestation corpus agrees 40/40 for both decode and verify.
  The gate also exposed stale release references in getting-started, security, and usage
  documentation; those now match the independently checked 0.6.0 registry release. These
  source fixes are unreleased and do not change that published archive or its checksum.
