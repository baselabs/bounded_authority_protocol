# Consumer seam note — CDC report path (non-normative)
...
The navyler_cdc platform (owner's private control plane) plans to carry this protocol on
its **CDC report path**: a customer-side edge agent — built from the CDC transport
libraries (`replicant`, `capstan`) — reports materializations, checkpoints, and events
