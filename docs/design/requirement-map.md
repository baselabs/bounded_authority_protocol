...
| REQ3-SIGNING-digest-prefix | Request-digest prefix `BAP3-REQUEST ` exact ASCII incl. final zero byte; archive prefix `BAP3-ARCHIVE EXPORT `; row hash domain `BAP3-CHAIN ` | request_digest, encode_consumption_entry, check_chain, encode_anchored_export | valid, exact_bound | populated | request_digest.valid=2, exact_bound=1 (digests are prefix-bound); check_chain.valid=1 and encode_anchored_export.valid=1 (BAP3-CHAIN/BAP3-ARCHIVE framing end to end); the v1/v2 archive and row cross-major rejects are prefix-caught. RED PROOF: mutation `v3-request-digest-prefix-downgraded` (BAP3→BAP1) → the CLI test fails |
