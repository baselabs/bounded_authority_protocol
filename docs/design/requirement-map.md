...
| REQ1-EVO-evolution-above-wire | Evolution via parallel majors, never in-place extension | `gap` | The v1 corpus contains only standard v1 artifacts; its input algebra cannot express an in-place extension to reject because the closed standard profile already rejects every unlisted member. The requirement is proven structurally by REQ1-CORE-reject-unlisted + the per-surface closed-set cells. A separately identified byte-distinct application profile is not an in-place evolution of standard v1: it carries a separate spec, identifier, API, and corpus and does not alter this map's standard-profile verdicts. |
...
## v3 — the contract-major 3 ES256 suite (`spec/bap-v3.md`, corpus `priv/conformance/v3/corpus`, revision 1)

The v3 profile activates the `BAP3-ES256-SHA256` suite ([ADR 0035](../adr/0035-es256-contract-major-activation.md)):
`alg: "ES256"` over EC P-256, the RFC 7518 §3.4 raw `r || s` signature form with the low-S
canonicality rule, EC JWKs and RFC 7638 EC thumbprints, uncompressed-SEC1 raw keys, and
`BAP3-*` domain separators, under the five-kind selector algebra inherited from v2. Its
requirement ids carry the `REQ3-*` range ([ADR 0007](../adr/0007-normative-requirement-identifiers.md));
cells cite the v3 index (`bounded-authority-protocol-v3-conformance-corpus-index`, 292 cases).
The v3 mutation gates' red proofs were executed against the battery
(`scripts/check_conformance_mutations.exs`, entries `v3-*`) on 2026-09-21 — each mutation was
applied, `mix test test/conformance/cli_test.exs` (the shipped-v3-corpus CLI agreement test)
observed failing, and the source restored byte-identical; the one entry the battery itself
adjudicated as not verdict-provable is disclosed in its row below.

| REQ-id | Requirement | Surface(s) | Cell class(es) | Cell-type | Evidence / reason + red proof |
|---|---|---|---|---|---|
| REQ3-CORE-cross-major-reject | A v3 verifier rejects every artifact whose `v` is not exactly 3 (all v1 AND v2 bytes); v1 and v2 reject v3 symmetrically; no fallback/downgrade | verify_grant, decode_grant, decode_proof, check_envelope, check_chain, verify_historical_anchor, verify_key_transition, verify_anchored_export | invalid_claim, invalid_encoding | populated | verify_grant.invalid_claim=2 (v1 bytes + v2 bytes); decode_grant.invalid_claim=2; decode_proof.invalid_claim=2; check_envelope.invalid_claim=2 (v1 grant and v1 proof); check_chain.invalid_claim=3 (v1 rows + v2 rows); verify_historical_anchor/verify_key_transition.invalid_claim=2 each (v1 + v2); verify_anchored_export.invalid_encoding=2 (v1 and v2 archives, prefix-bound). RED PROOF: mutation `v3-cross-major-grant-header-and-v-accepted` (grant decode accepts the EdDSA header and `v:1`/`v:2`) → the shipped-v3-corpus CLI test fails; battery-adjudicated composite — v3 cross-major rejection is pinned at both the alg and the v claim |
| REQ3-CORE-v1-incorporation | The v1 normative sections are incorporated with the enumerated substitutions (spec/bap-v3.md §2) and the v2 selector algebra by reference | (all 28 surfaces) | — | populated | every surface carries at least one populated cell in the v3 index; the version-neutral primitive surfaces execute the identical shared modules via byte-carried v1 cells |
| REQ3-CLAIM-v; REQ3-CLAIM-proof-v | Grant/proof `v` MUST be exactly integer 3 | verify_grant, check_envelope | invalid_claim | populated | see REQ3-CORE-cross-major-reject cells |
| REQ3-EVO-proof-major-equals-grant; REQ3-EVO-mixed-major-invalid | A v3 proof MUST pair with a v3 grant; mixed-major credentials are invalid by construction | check_envelope | invalid_claim | populated | the check_envelope cross-major pair: `check-envelope-v3-invalid-cross-major-v1-grant` (v3 proof over a v1 grant) and `check-envelope-v3-invalid-cross-major-v1-proof` (v1 proof over a v3 grant); the v2-bytes twins live on the decode surfaces |
| REQ3-HEADER-alg | Protected-header `alg` is exactly `"ES256"` on every v3 artifact | decode_grant, decode_proof, verify_grant, check_envelope, verify_historical_anchor, verify_key_transition | invalid_algorithm | populated | decode_grant/decode_proof/verify_grant.invalid_algorithm=1 each; check_envelope.invalid_algorithm=2 (grant-side and proof-side none); anchor and transition .invalid_algorithm=1 each. RED PROOF (suite-level): mutations `v3-signature-low-s-removed` and `v3-cross-major-grant-v-accepted` both reddened via the CLI test; the alg pin itself is load-borne by every none-alg cell above |
| REQ3-HEADER-closed-set | Header member sets are exactly the closed sets (`{alg,kid,typ}` grants/anchors/transitions, `{alg,jwk,typ}` proofs); payload cnf is exactly `{jkt}` | decode_grant, decode_proof, check_envelope | invalid_encoding, invalid_claim | populated | check_envelope.invalid_encoding=1 (`check-envelope-v3-invalid-encoding-cnf-extra-member` — cnf with an extra member); the framing-defect decode cells (1 segment) exercise the header/payload split |
| REQ3-HEADER-digest-width | Every digest-shaped member (`cnf.jkt`, `ath`, `ba_req`, `chain_hash`, fingerprints) is exactly 32 bytes, base64url-encoded | verify_grant, check_envelope, jwk.thumbprint_raw | valid, invalid_encoding | populated | the valid cells carry 32-byte thumbprints/hashes end to end; jwk thumbprint surfaces reject non-digest shapes (invalid_encoding=1 each) |
| REQ3-HEADER-no-private-jwk | The proof JWK never carries private material (`d`) or extra members | jwk.decode_public | invalid_key | populated | `jwk-decode-public-invalid-extra-member-d` (jwk.decode_public.invalid_key=8 includes the `d`-member case) |
| REQ3-HEADER-proof-jwk | The proof holder key is the EC JWK exactly `{"crv":"P-256","kty":"EC","x":…,"y":…}` | decode_proof, check_envelope, jwk.decode_public | valid, invalid_key | populated | decode_proof.valid=1 and check_envelope.valid=6 carry the EC JWK in every proof header; the OKP-shape and wrong-crv JWK rejects live in jwk.decode_public.invalid_key=8. RED PROOF: mutation `v3-jwk-curve-accepted` (any crv string decodes) → the CLI test fails |
| REQ3-HEADER-thumbprint | The holder binding is the RFC 7638 EC thumbprint over `{crv,kty,x,y}` (lexicographic member order, unpadded coordinates) | jwk.thumbprint, jwk.thumbprint_raw, jwk.thumbprint_preimage, jwk.public_key_thumbprint_raw | valid, invalid_encoding, tamper_meaningful_byte | populated | jwk.thumbprint/.thumbprint_raw/.thumbprint_preimage each: valid=1, invalid_encoding=1, tamper_meaningful_byte=1; jwk.public_key_thumbprint_raw.valid=1; the OKP-member preimage and member-order rejects in jwk.decode_public. RED PROOF: mutation `v3-thumbprint-member-set-wrong` (preimage without `y`) → the CLI test fails |
| REQ3-KEY-point-on-curve | A decoded JWK point MUST satisfy the curve equation (pure arithmetic y² = x³ − 3x + b mod p) | jwk.decode_public | invalid_key | populated | `jwk-decode-public-invalid-off-curve` (jwk.decode_public.invalid_key=8 includes it) |
| REQ3-KEY-uncompressed-sec1 | Raw public keys are exactly the 65-byte uncompressed SEC1 form `0x04 ‖ x ‖ y` | jwk.encode_public, jwk.public_key_thumbprint_raw, boundary_anchor_signing_input | invalid_key, valid | populated | jwk.encode_public.invalid_key=2 (`invalid-length-64`, `invalid-compressed-form`); jwk.public_key_thumbprint_raw.invalid_key=1 (short key); boundary_anchor_signing_input.invalid_key rejects short anchor keys |
| REQ3-SIGNING-raw-rs | The wire signature is exactly 64 bytes: fixed-width 32-byte unsigned big-endian `r ‖ s` (no DER on the wire) | verify_grant, decode_grant, decode_proof, assemble_compact | valid, invalid_encoding, tamper_meaningful_byte | populated | every valid cell verifies the 64-byte form; verify_grant.invalid_encoding=6 (the canonicality matrix — see below); assemble_compact.invalid_key rejects short signatures; verify_grant.tamper_meaningful_byte=3 includes the tampered-`r` and tampered-`s` legs |
| REQ3-SIGNING-low-s | `s` MUST be ≤ n/2 (low-S); a high-S signature is invalid even though raw ECDSA accepts it | verify_grant | invalid_encoding | populated | `verify-grant-v3-invalid-signature-high-s` (inside verify_grant.invalid_encoding=6) — a genuinely verifying high-S signature (backend malleability, probe-observed). RED PROOF: mutation `v3-signature-low-s-removed` (half-order rule dropped) → the high-S case agrees as valid → the CLI test fails |
| REQ3-SIGNING-range | `0 < r < n` and `0 < s ≤ n/2`; zero and out-of-range scalars invalid | verify_grant | invalid_encoding | populated | `verify-grant-v3-invalid-signature-{zero-r,zero-s,r-at-n,s-at-n}` (inside verify_grant.invalid_encoding=6) all reject. RED-PROOF ADJUDICATION: mutation `v3-signature-range-removed` (r < n dropped) SURVIVED the battery — the backend independently rejects out-of-range scalars mathematically, so no corpus verdict isolates the check; the check's load-bearing proof is therefore unit-level (`valid_raw_signature?` property tests in the v3 suite), disclosed here rather than overstated. The zero-scalar legs are caught by the surviving `> 0` checks under the same adjudication |
| REQ3-SIGNING-backend-reject | Backend rejection or exception (off-curve point raises at :crypto.verify) returns exactly closed invalid, never a crash | verify_grant, check_envelope, jwk.decode_public | invalid_key, tamper_meaningful_byte | populated | verify_grant.tamper_meaningful_byte=3 and check_envelope.tamper_meaningful_byte=1 (backend rejects wrong-math signatures); the off-curve JWK is rejected BEFORE the backend by the pure-arithmetic on-curve check (REQ3-KEY-point-on-curve), and the exception path is unit-proven (rescue clause) |
| REQ3-SIGNING-digest-prefix | Request-digest prefix `BAP3-REQUEST\0` exact ASCII incl. final zero byte; archive prefix `BAP3-ARCHIVE\0EXPORT\0`; row hash domain `BAP3-CHAIN\0` | request_digest, encode_consumption_entry, check_chain, encode_anchored_export | valid, exact_bound | populated | request_digest.valid=2, exact_bound=1 (digests are prefix-bound); check_chain.valid=1 and encode_anchored_export.valid=1 (BAP3-CHAIN/BAP3-ARCHIVE framing end to end); the v1/v2 archive and row cross-major rejects are prefix-caught. RED PROOF: mutation `v3-request-digest-prefix-downgraded` (BAP3→BAP1) → the CLI test fails |
| REQ3-BOUNDS-inherited | Every v1 bound carries over unchanged; no new bound | bounds.new | exact_bound, maximum_plus_one, invalid_limit | populated | bounds.new.exact_bound=38, maximum_plus_one=33, invalid_limit=6 (byte-carried v1 cells; the bounds module is shared). The suite's own fixed-width rules (65-byte keys, 64-byte signatures, 32-byte coordinates) are ADR 0035 §5's inert-width decision, not new Bounds entries |
| REQ3-BOUNDS-fixed-widths | The EC fixed widths: 65-byte raw keys, 32-byte coordinates, 64-byte raw signatures, 32-byte digests | jwk.decode_public, jwk.encode_public, assemble_compact | invalid_key, invalid_encoding | populated | short/padded-coordinate rejects (jwk.decode_public.invalid_key=8); length-64/compressed key rejects (jwk.encode_public.invalid_key=2); short-signature reject (assemble_compact) |
| REQ3-BOUNDS-ordering | Raw/encoded size checks precede decode; width checks precede the backend call; all precede crypto (the v1 ordering under EC shapes) | bounds.new, json.decode, decode_grant, jwk.decode_public | exact_bound, maximum_plus_one, invalid_key | populated | bounds.new.exact_bound=38, maximum_plus_one=33; json.decode byte-carried exact_bound cells; the short-coordinate JWK is rejected at width before any curve arithmetic |
| REQ3-CORPUS-certified-identity | The v3 corpus is a certified artifact with its own pinned index identity | (CLI + SDK runners) | — | populated | certified SHA-256 pinned in the CLI (major-keyed map), the CLI test, and every SDK v3 runner; `mix conformance.verify` runs all three corpora (v1 = 283 unchanged pin, v2 = 268 unchanged pin, v3 = 292) |
...
- **v2 (ADR 0030 activation):** 22 `REQ2-*` ids over `spec/bap-v2.md`; all map to populated
  ...
  surfaces, 16 classes).
- **v3 (ADR 0035 activation):** 23 `REQ3-*` ids over `spec/bap-v3.md`; all map to populated
  v3 corpus cells, with one disclosed red-proof adjudication (`REQ3-SIGNING-range`: the
  range check is backend-coincident for out-of-range scalars, so its load-bearing proof is
  unit-level — the corpus cells prove the rejections, not sole-rejection). Corpus:
  `bounded-authority-protocol-v3-conformance-corpus-index` (292 cases, 28 surfaces,
  16 classes).
