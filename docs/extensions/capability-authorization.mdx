...
servers advertise support for this extension in the `initialize` request/response. The per-extension
settings object is an *example* of the deployment's accepted cryptographic suite and contract-major;
its exact shape is not fixed by this draft (the Bounded Authority Protocol reserves a "verifier
discovery document" for the deployment-published set of accepted contract-majors and suites, with
its exact name and shape fixed at first external submission — see the
[registries](https://github.com/baselabs/bounded_authority_protocol/blob/main/docs/design/registries.md)
§ reserved names). An empty settings object indicates no settings:
...
(The OAuth-scope-based authorization in the `ext-auth` extensions is itself an optional extension,
not the core behavior; a deployment that layers this extension on top of an `ext-auth` extension
reverts to whichever authorization posture it had before adding capability-auth.) A server that
