defmodule BoundedAuthorityProtocol.V3.Runtime do
  @moduledoc false
  ...
  defp parse_proof_validated(compact, limits) do
    with {:ok, bounds} <- Bounds.coerce(limits),
         ...
         {:ok, payload} <- closed_proof_payload(payload_members),
  ...
  defp decode_proof_fields(header, payload, bounds) do
    with {:string, "ES256"} <- header["alg"],
         ...
         {:ok, holder_public_key} <- EcJwk.decode_public(jwk_bytes, bounds),
         {:ok, holder_thumbprint} <- EcJwk.thumbprint_raw(jwk_bytes, bounds),
         {:integer, 3} <- payload["v"],
  ...
  defp proof_json(proof, bounds) do
    with true <-
           is_binary(proof.holder_public_key) and
             byte_size(proof.holder_public_key) == @public_key_bytes,
         {:ok, jwk_bytes} <- EcJwk.encode_public(proof.holder_public_key, bounds),
  ...
  defp verify_grant_parsed(parsed, trusted, expected, bounds) do
    ...
    with true <- secure_equal?(grant.key_id, trusted.key_id),
         ...
         {:ok, _fingerprint} <- EcJwk.public_key_thumbprint_raw(trusted.public_key, bounds) do
      :ok
    else
      _failure -> {:error, :invalid}
    end
  ...
  defp validate_trusted_issuer(%TrustedIssuer{} = trusted, bounds) do
    if valid_key_id?(trusted.key_id, bounds) and is_binary(trusted.public_key) and
         byte_size(trusted.public_key) == @public_key_bytes do
      :ok
    else
      {:error, :invalid}
    end
  ...
  defp grant_facts(grant, public_key, audience, bounds) do
    {:ok, fingerprint} = EcJwk.public_key_thumbprint_raw(public_key, bounds)
  ...
  defp envelope_facts(grant_parsed, proof_parsed, expected, bounds) do
    ...
    {:ok, fingerprint} =
      EcJwk.public_key_thumbprint_raw(expected.trusted_issuer.public_key, bounds)
  ...
  defp encode_selector({:lte, path, bound}, bounds) do
    with true <- valid_path?(path, bounds),
  ...
  defp encode_selector({:gte, path, bound}, bounds) do
    with true <- valid_path?(path, bounds),
         true <- numeric_bound?(bound),
         {:ok, _encoded} <- Jcs.encode(bound, bounds) do
      {:ok,
  ...
  defp closed_proof_payload(members) do
  ...
  defp verify_signature(message, signature, public_key) do
    is_binary(message) and is_binary(signature) and is_binary(public_key) and
      byte_size(public_key) == @public_key_bytes and
      Es256.verify(message, signature, public_key)
  end
