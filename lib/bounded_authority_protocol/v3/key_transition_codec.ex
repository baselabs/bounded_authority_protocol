defmodule BoundedAuthorityProtocol.V3.KeyTransitionCodec do
  @moduledoc false

  @public_key_bytes 65

  alias BoundedAuthorityProtocol.V3.Es256
  ...
  alias BoundedAuthorityProtocol.V3.ContextValidation
  ...
  def signing_input(%KeyTransition{} = transition, limits) do
    with {:ok, bounds} <- Bounds.coerce(limits),
         ...
         {:ok, current_fingerprint} <-
           EcJwk.public_key_thumbprint_raw(transition.current_public_key, bounds),
         {:ok, next_fingerprint} <-
           EcJwk.public_key_thumbprint_raw(transition.next_public_key, bounds),
  ...
  def verify(
    ...
    with {:ok, bounds} <- Bounds.coerce(expected.bounds),
         ...
         {:ok, current_fingerprint} <-
           EcJwk.public_key_thumbprint_raw(current_key.public_key, bounds),
         {:ok, next_fingerprint} <- EcJwk.public_key_thumbprint_raw(next_key.public_key, bounds),
         ...
         true <- Es256.verify(parsed.message, parsed.signature, current_key.public_key) do
      {:ok,
       %KeyTransitionFacts{
         version: 3,
  ...
  defp validate_transition_input(transition, bounds) do
    with true <- is_binary(transition.current_public_key),
         true <- byte_size(transition.current_public_key) == @public_key_bytes,
         true <- is_binary(transition.next_public_key),
         true <- byte_size(transition.next_public_key) == @public_key_bytes,
         :ok <-
           validate_transition_fields(
