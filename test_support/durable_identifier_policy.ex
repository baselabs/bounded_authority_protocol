defmodule BoundedAuthorityProtocol.TestSupport.DurableIdentifierPolicy do
  ...
  defp contract_identity?(path, :wire_suite, suite)
       when suite in ["BAP1-Ed25519-SHA256", "BAP2-Ed25519-SHA256", "BAP3-ES256-SHA256"],
  ...
  defp contract_identity?(path, :wire_field, field)
       when field in [~s("v": 1), ~s("v": 2), ~s("v": 3)],
       do: contract_content_surface?(path)

  defp contract_identity?(path, :external_wire_module, namespace)
       when namespace in [
              "BoundedAuthorityProtocol.V1",
              "BoundedAuthorityProtocol.V2",
              "BoundedAuthorityProtocol.V3"
            ],
  ...
  defp contract_identity?(path, :requirement_id, name) do
    case Regex.run(~r/\AREQ([123])-([A-Z0-9]+)-[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\z/, name) do
      [_, _major, surface] ->
  ...
  defp contract_identity?(path, :module, name) do
    ((name == "BoundedAuthorityProtocol.V1" or
      ...
      ((name == "BoundedAuthorityProtocol.V3" or
          String.starts_with?(name, "BoundedAuthorityProtocol.V3.")) and
         es256_major_source_path?(path)) or
      (String.starts_with?(
         name,
  ...
  defp current_major_path?(path, "v3"), do: es256_major_source_path?(path)

  defp current_major_path?(path, "run_v3")
       when path in ["sdks/python/tests/conformance/run_v3.py", "sdks/rust/conformance/run_v3.rs"],
       do: true

  defp current_major_path?(path, "v3_test"), do: path == "sdks/go/conformance/v3_test.go"

  defp current_major_path?(path, "v3_permissiveness_internal_test"),
    do: path == "sdks/go/v3_permissiveness_internal_test.go"

  defp current_major_path?(path, "corpus-v3"),
    do:
      String.starts_with?(path, "sdks/go/conformance/corpus-v3/") or
        String.starts_with?(path, "sdks/rust/conformance/corpus-v3/")
  ...
  defp current_major_path?(path, name) do
    {path, name} in [
      ...
      {"spec/bap-v3.md", "bap-v3"},
      {"docs/adr/0035-es256-contract-major-activation.md",
       "0035-es256-contract-major-activation"},
  ...
  # The accepted ES256 contract-major families (ADR 0035 activation): the V3 namespace, the
  # v3 corpus, and the v3 SDK profiles/runners.
  defp es256_major_source_path?(path) do
    Regex.match?(~r"\Alib/bounded_authority_protocol/v3(?:\.ex|/)", path) or
      String.starts_with?(path, "test/bounded_authority_protocol/v3/") or
      String.starts_with?(path, "priv/conformance/v3/") or
      path in [
        "sdks/go/v3.go",
        "sdks/go/v3_permissiveness_internal_test.go",
        "sdks/go/conformance/v3_test.go",
        "sdks/python/src/bounded_authority_verifier/v3.py",
        "sdks/python/tests/conformance/run_v3.py",
        "sdks/rust/src/v3.rs",
        "sdks/rust/conformance/run_v3.rs",
        "conformance/generators/build_corpus.mjs",
        "conformance/generators/curated-inputs-v3.json"
      ]
  end
